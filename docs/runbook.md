...
first if you orchestrate manually. Always take a backup (below) before a major upgrade. A first boot on an empty database
logs no errors since v0.59.2 (the API creates the migrations-history table before EF Core looks for it); an `Error`
line from `Microsoft.EntityFrameworkCore.Database.Command` at startup is therefore always worth reading.
