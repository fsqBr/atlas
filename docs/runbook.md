...
- Cost sources are admin operations: store the provider key under `PUT /api/credentials/{name}`, connect it with
  `PUT /api/ai-estate/cost/sources/{openai|anthropic|github-copilot}` (`credentialName`, `scope` = GitHub org for Copilot),
  pull with `POST /api/ai-estate/cost/sync`. Sync is manual and read-only; schedule it from your CI/cron if you want it
  daily. Results and the last error per provider are visible in `GET /api/ai-estate/cost/sources`.
