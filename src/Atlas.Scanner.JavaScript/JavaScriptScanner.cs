...
public sealed partial class JavaScriptScanner : IScanner
{
    ...
    private static bool IsVendorOrGenerated(string path)
    {
        ...
        return lower.Contains("/node_modules/") || lower.Contains("/bower_components/") || lower.Contains("/wwwroot/lib/") || lower.Contains("/scripts/lib/")
            || lower.Contains("/dist/") || lower.Contains("/vendor/") || lower.EndsWith(".min.js") || lower.EndsWith(".bundle.js") || IsJqueryLibraryFile(lower) || lower.Contains("/typings/") || lower.EndsWith(".d.ts");
    }

    /// <summary>jquery-3.7.1.js / jquery.min.js / jquery-ui.js are the vendored library; jquery.myPlugin.js is
    /// the classic hand-written plugin naming in legacy ASP.NET and must be scanned like any other source file.</summary>
    private static bool IsJqueryLibraryFile(string lowerPath)
    {
        var file = lowerPath[(lowerPath.LastIndexOf('/') + 1)..];
        if (!file.StartsWith("jquery", StringComparison.Ordinal))
        {
            return false;
        }

        var rest = file["jquery".Length..];
        return rest is ".js"
            || rest.StartsWith("-", StringComparison.Ordinal)
            || (rest.Length > 1 && rest[0] == '.' && (char.IsDigit(rest[1])
                || rest.StartsWith(".ui", StringComparison.Ordinal)
                || rest.StartsWith(".min", StringComparison.Ordinal)
                || rest.StartsWith(".slim", StringComparison.Ordinal)
                || rest.StartsWith(".migrate", StringComparison.Ordinal)
                || rest.StartsWith(".validate", StringComparison.Ordinal)
                || rest.StartsWith(".unobtrusive", StringComparison.Ordinal)
                || rest.StartsWith(".signalr", StringComparison.Ordinal)));
    }
