...
public sealed partial class PythonScanner(IVulnerabilitySource vulnerabilities) : IScanner
{
    ...
    public async Task<ScanResult> ExecuteAsync(ScanContext context, CancellationToken cancellationToken)
    {
        ...
        foreach (var (pattern, parser) in new (string, Func<string, string, PythonModule>)[]
                 {
                     ("requirements.txt", PythonManifests.ParseRequirements),
                     ("pyproject.toml", PythonManifests.ParsePyProject),
                     ("Pipfile", PythonManifests.ParsePipfile),
                     ("setup.py", PythonManifests.ParseSetupPy),
                 })
        {
        ...
        var pythonLanguage = context.Languages.GetValueOrDefault(LanguageIds.Python);
    ...
    private static Severity MapOsvSeverity(string? severity) => OsvSeverity.Map(severity);
