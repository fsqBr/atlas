...
public sealed class ModernizationActual
{
    ...
    /// <summary>The cost.v1 likely-hours estimate frozen when the actual was recorded: calibration must
    /// compare against what was promised, not against a plan recomputed after the modernization resolved
    /// the very findings that drove the estimate.</summary>
    ...
    public ModernizationActual(Guid assessmentId, Guid tenantId, ModernizationStrategy strategy, double actualHours, double? actualMonths, decimal? actualCost, string currency, string? notes, string recordedBy, double? estimatedHours = null)
    {
        ...
        TenantId = tenantId;
        Update(strategy, actualHours, actualMonths, actualCost, currency, notes, recordedBy, estimatedHours);
    }

    public void Update(ModernizationStrategy strategy, double actualHours, double? actualMonths, decimal? actualCost, string currency, string? notes, string recordedBy, double? estimatedHours = null)
    {
        if (actualHours <= 0)
        {
