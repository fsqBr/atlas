...
public sealed class SuppressionPolicy
{
    ...
    /// <summary>Optional waiver end: after this instant the policy stops filtering candidates,
    /// so the findings it silenced come back on the next run.</summary>
    public DateTimeOffset? ExpiresAtUtc { get; private set; }

    public bool IsActive(DateTimeOffset now) => ExpiresAtUtc is null || ExpiresAtUtc > now;

    private SuppressionPolicy()
    {
    }

    public SuppressionPolicy(Guid id, Guid tenantId, Guid? assessmentId, string rulePattern, string? pathGlob, string reason, string author, DateTimeOffset? expiresAtUtc = null)
    {
