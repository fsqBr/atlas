...
public sealed class CSharpLanguageAnalyzer(RestoredReferences? tier2 = null) : ILanguageAnalyzer
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (trees.Count > 0)
        {
            ...
            {
                cancellationToken.ThrowIfCancellationRequested();
                var stats = ResolveSymbolsSample(compilation, projectTrees, cancellationToken);
                sampled += stats.SampledInvocations;
                resolved += stats.ResolvedInvocations;
                foreach (var tree in projectTrees)
                {
                    patterns.AddRange(ObsoleteApiDetector.Collect(compilation, tree, tree.FilePath, cancellationToken));
                }

                namespaceDependencies.AddRange(NamespaceGraphBuilder.Build(compilation, projectTrees, cancellationToken));
            }
    ...
    private static SymbolResolutionStats ResolveSymbolsSample(
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var tree in trees)
        {
            ...
            {
                if (sampled >= SymbolSampleSize)
                {
                    break;
                }

                sampled++;
                if (semanticModel.GetSymbolInfo(invocation, cancellationToken).Symbol is not null)
                {
                    resolved++;
                }
            }
