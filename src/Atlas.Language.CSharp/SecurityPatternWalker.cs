...
/// </summary>
public sealed class SecurityPatternWalker(string filePath) : CSharpSyntaxWalker
{
    ...
    private static readonly HashSet<string> WeakHashFactories = new(StringComparer.Ordinal) { "MD5.Create", "SHA1.Create", "MD5.HashData", "SHA1.HashData" };
    ...
    /// <summary>Target-typed `BinaryFormatter bf = new();` deserves the same finding as the explicit form.</summary>
    public override void VisitImplicitObjectCreationExpression(ImplicitObjectCreationExpressionSyntax node)
    {
        var declaredType = node.FirstAncestorOrSelf<VariableDeclarationSyntax>()?.Type;
        if (declaredType is not null)
        {
            var typeName = SimpleName(declaredType);
            if (typeName == "BinaryFormatter")
            {
                Report(SecurityPatternIds.BinaryFormatter, node, "new BinaryFormatter()");
            }
    ...
    /// <summary>
    /// Last <paramref name="segments"/> dotted segments of an expression text. Conditional access yields a
    /// member binding whose text starts with '.' ("?.Create()" → ".Create"); an empty text is possible too —
    /// both used to push the search index below zero and crash the whole scan host (v0.48–v0.49).
    /// </summary>
    private static string TailSegments(string text, int segments)
    {
        var index = text.Length;
        for (var i = 0; i < segments && index > 0; i++)
        {
            var previous = text.LastIndexOf('.', index - 1);
            if (previous < 0)
            {
                return text;
            }
        ...
        return index >= text.Length ? string.Empty : text[(index + 1)..];
    }

    public override void VisitInvocationExpression(InvocationExpressionSyntax node)
    {
        // Compare on the last two segments so System.Security.Cryptography.MD5.Create() matches
        // like MD5.Create() — exact-string matching missed every namespace-qualified call.
        var callee = TailSegments(node.Expression.ToString(), 2);
    ...
    {
        var text = TailSegments(node.ToString(), 2);
