...
public sealed record ExecutiveReport(
    ...
    ReportAiEstate? AiEstate = null);

/// <summary>
/// The AI Estate section (governance module V0.5): the repository's persisted AI inventory plus the
/// correlations with the same repository's open privacy and secrets findings. Deterministic — read from
/// findings, never recomputed. <see cref="Scanned"/> false = the AI scanner did not run on this assessment.
/// </summary>
public sealed record ReportAiEstate(
    bool Scanned,
    Atlas.Application.AiEstate.AiEstateRecord? Record,
    int OpenPii,
    int OpenSecrets,
    IReadOnlyList<ReportRuleGroup> AiRuleGroups,
    IReadOnlyList<ReportCostLine>? Costs = null);

/// <summary>One line of AI spend for the report: provider, dimension, period, amount and its basis (ProviderReported vs Estimated — never summed together).</summary>
public sealed record ReportCostLine(string Provider, string Dimension, string Period, decimal Amount, string Currency, string Basis);
...
public sealed record ReportModernization(
    ...
    ReportSavings? Savings = null);

public sealed record ReportSavings(
    IReadOnlyList<(string Label, decimal AnnualAmount)> Items,
    decimal AnnualTotal,
    string Currency,
    double? RecommendedPaybackMonths);

public sealed record ReportStrategy(string Name, int FitScore, string Risk, bool Recommended, double LikelyHours, double LikelyMonths, decimal LikelyCost, string Currency, IReadOnlyList<string> Rationale, IReadOnlyList<string> Blockers, double? PaybackMonths = null);
