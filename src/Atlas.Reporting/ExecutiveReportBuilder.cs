    ...
    Atlas.Application.Ai.IAiNarrativeRepository narratives)
{
    ...
    public async Task<ExecutiveReport?> BuildAsync(Guid assessmentId, ReportLocale locale, CancellationToken cancellationToken, DateTimeOffset? since)
    {
        ...
        var recoveredRules = (await businessRules.ListAsync(assessmentId, cancellationToken))
            ...
            .ToList();
        var aiEstate = BuildAiEstate(latestScans, page.Items, open, groups);
    ...
    /// <summary>AI Estate from persisted facts: the ai.inventory record plus what else is open in the same repository.</summary>
    internal static ReportAiEstate BuildAiEstate(
        IReadOnlyList<ReportScan> latestScans,
        IReadOnlyList<FindingWithLatestOccurrence> items,
        IReadOnlyList<ReportFinding> open,
        IReadOnlyList<ReportRuleGroup> groups)
    {
        var scanned = latestScans.Any(s => s.ScannerId == AiEstateRuleIds.ScannerId);
        var inventory = items.FirstOrDefault(i => i.Finding.RuleId == AiEstateRuleIds.Inventory && i.Finding.Status is FindingStatus.Open or FindingStatus.Regressed);
        var record = AiEstateRecord.Parse(inventory?.Latest?.DataJson);
        return new ReportAiEstate(
            Scanned: scanned || record is not null,
            Record: record,
            OpenPii: open.Count(f => f.RuleId.StartsWith(AiEstateRuleIds.PiiPrefix, StringComparison.Ordinal)),
            OpenSecrets: open.Count(f => f.Category == FindingCategory.Secrets && !AiEstateRuleIds.IsAiRule(f.RuleId)),
            AiRuleGroups: groups.Where(g => AiEstateRuleIds.IsAiRule(g.RuleId) && g.RuleId != AiEstateRuleIds.Inventory).ToList());
    }
    ...
    private async Task<ReportModernization?> BuildModernizationAsync(Guid assessmentId, string lang, CancellationToken cancellationToken)
    {
        ...
        var savings = plan.Savings is null ? null : new ReportSavings(
            plan.Savings.Items.Select(i => (ModernizationTexts.Text(i.Key, lang), i.AnnualAmount)).ToList(),
            plan.Savings.AnnualTotal,
            plan.Savings.Currency,
            plan.PaybackMonths(recommended));
        ...
        return new ReportModernization(
            ...
            $"{plan.Assessment.ModelVersion} · {recommended.ModelVersion} · {plan.Roadmap.ModelVersion}{(plan.Savings is null ? "" : " · " + plan.Savings.ModelVersion)}",
            savings);
    }
}
