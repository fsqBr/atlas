...
public static class DependencyInjection
{
    ...
    public static IServiceCollection AddAtlasGovernance(this IServiceCollection services, IConfiguration configuration, string connectionString)
    {
        ...
        services.AddScoped<ITelemetryStreamRepository, TelemetryStreamRepository>();
        services.AddScoped<IBudgetRepository, BudgetRepository>();
        services.AddScoped<ITeamRepository, TeamRepository>();
        services.AddScoped<BudgetService>();
        services.AddScoped<TeamService>();
        services.AddScoped<ReconciliationService>();
        services.AddScoped<TelemetryIngestService>();
        services.AddSingleton(new TelemetryOptions
        {
            ActorMode = configuration["Atlas:AiEstate:Telemetry:ActorMode"] ?? "pseudonym",
            HmacKeyBase64 = configuration["Atlas:Secrets:HmacKeyBase64"],
        });
        services.AddHttpClient(WebhookAlertSink.HttpClientName, http => http.Timeout = TimeSpan.FromSeconds(10));
        services.AddScoped<IAlertSink, WebhookAlertSink>();
        ...
        services.TryAddEnumerable(ServiceDescriptor.Singleton<ICostProviderClient, CursorClient>());
        return services;
    }
}
