...
public sealed class SecretsScanner : IScanner
{
    ...
    private static async Task<bool> LooksBinaryAsync(Stream stream, CancellationToken cancellationToken)
    {
        ...
        if (read >= 2 && ((span[0] == 0xFF && span[1] == 0xFE) || (span[0] == 0xFE && span[1] == 0xFF)))
        {
            return false; // UTF-16 text (PowerShell redirects save config files like this); the reader honors the BOM
        }
