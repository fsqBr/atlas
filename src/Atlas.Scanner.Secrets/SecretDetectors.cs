...
public static class SecretDetectors
{
    ...
    [
        ...
        // The name may carry any prefix (db_password, dbPassword, aws_secret_access_key) and may be a quoted
        // JSON/YAML key; a bare \b before the keyword never matches '_' or camelCase prefixes.
        new("secrets.generic-assignment", "Hard-coded credential assignment", "Credencial fixa em atribuição", Severity.Medium, ConfidenceLevel.Low,
            new Regex(@"(?i)(?<![\w.-])[\w.-]{0,40}?(?:api[_-]?key|apikey|client[_-]?secret|access[_-]?token|auth[_-]?token|secret[_-]?access[_-]?key|secret[_-]?key|password|passwd|pwd)[""']?\s*[:=]\s*[""']([^""']{8,})[""']", Opts), true,
        ...
        // Unquoted assignments: .env files, YAML and docker-compose environment lines.
        new("secrets.generic-assignment-unquoted", "Hard-coded credential assignment (unquoted)", "Credencial fixa em atribuição (sem aspas)", Severity.Medium, ConfidenceLevel.Low,
            new Regex(@"(?i)^\s*(?:export\s+|-\s+)?[\w.-]{0,40}?(?:api[_-]?key|apikey|client[_-]?secret|access[_-]?token|auth[_-]?token|secret[_-]?access[_-]?key|secret[_-]?key|password|passwd|pwd)\s*[=:]\s*([^\s""'#$%{][^\s#]{7,})\s*$", Opts), true,
            "Load credentials from configuration/secret stores; never commit literal values.",
            "Carregue credenciais de configuração/cofres de segredos; nunca faça commit de valores literais."),

        new("secrets.anthropic-api-key", "Anthropic API key", "Chave de API da Anthropic", Severity.High, ConfidenceLevel.High,
            new Regex(@"\b(sk-ant-[A-Za-z0-9_-]{20,})\b", Opts), false,
            "Revoke the key in the Anthropic console and load it from a secret store.",
            "Revogue a chave no console da Anthropic e carregue-a de um cofre de segredos."),

        new("secrets.openai-api-key", "OpenAI API key", "Chave de API da OpenAI", Severity.High, ConfidenceLevel.High,
            new Regex(@"\b(sk-proj-[A-Za-z0-9_-]{20,}|sk-[A-Za-z0-9]{20,}T3BlbkFJ[A-Za-z0-9]{20,})\b", Opts), false,
            "Revoke the key in the OpenAI dashboard and load it from a secret store.",
            "Revogue a chave no dashboard da OpenAI e carregue-a de um cofre de segredos."),

        new("secrets.gitlab-token", "GitLab personal access token", "Token de acesso pessoal do GitLab", Severity.High, ConfidenceLevel.High,
            new Regex(@"\b(glpat-[A-Za-z0-9_-]{20,})\b", Opts), false,
            "Revoke the token in GitLab settings and use CI/CD variables or a secret store.",
            "Revogue o token nas configurações do GitLab e use variáveis de CI/CD ou um cofre de segredos."),

        new("secrets.npm-token", "npm access token", "Token de acesso do npm", Severity.High, ConfidenceLevel.High,
            new Regex(@"\b(npm_[A-Za-z0-9]{36})\b", Opts), false,
            "Revoke the token on npmjs.com and use scoped automation tokens from a secret store.",
            "Revogue o token no npmjs.com e use tokens de automação com escopo a partir de um cofre de segredos."),
    ];
