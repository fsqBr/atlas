...
public sealed record TeamResponse(Guid Id, string Name, IReadOnlyList<string> Members, string UpdatedBy, DateTimeOffset UpdatedAtUtc);

public sealed record UpsertTeamRequest(Guid? Id, string Name, IReadOnlyList<string> Members, string? Author = null);
