...
public sealed class ArchitectureScanner : IScanner
{
    ...
    private static void EmitChangeRisk(ScanContext context, IReadOnlyList<FileFact> files)
    {
        ...
        var months = context.History.Where(h => h.LastChangeUtc is not null).Select(h => h.LastChangeUtc!.Value).DefaultIfEmpty(DateTimeOffset.UtcNow).Min();
        var window = Math.Max(1, (int)Math.Round((DateTimeOffset.UtcNow - months).TotalDays / 30.0));
