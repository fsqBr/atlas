...
public static class DependencyInjection
{
        ...
        string connectionString)
    {
        ...
        services.AddScoped<Atlas.Application.AiEstate.ITenantAiEstateSettingsRepository, TenantAiEstateSettingsRepository>();
        ...
        // Built eagerly so a malformed master key fails the process at startup with a clear message
        // instead of surfacing as HTTP 500 on the first credential read.
        var secretOptions = configuration.GetSection(SecretCipherOptions.SectionName).Get<SecretCipherOptions>() ?? new SecretCipherOptions();
        services.AddSingleton(secretOptions);
        services.AddSingleton<ISecretCipher>(new AesGcmSecretCipher(secretOptions));
