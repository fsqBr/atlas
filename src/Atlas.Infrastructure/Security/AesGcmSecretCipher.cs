...
public sealed class AesGcmSecretCipher : ISecretCipher
{
    ...
    public AesGcmSecretCipher(SecretCipherOptions options)
    {
        ...
        try
        {
            key = Convert.FromBase64String(options.MasterKeyBase64.Trim());
        }
        catch (FormatException)
        {
            throw new InvalidOperationException(
                "Atlas:Secrets:MasterKeyBase64 (ATLAS_MASTER_KEY) is not valid base64. Generate it with: openssl rand -base64 32");
        }

        if (key.Length != 32)
        {
            throw new InvalidOperationException(
                $"Atlas:Secrets:MasterKeyBase64 (ATLAS_MASTER_KEY) must decode to exactly 32 bytes (AES-256); it decodes to {key.Length}. Generate it with: openssl rand -base64 32");
        }
