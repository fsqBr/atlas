...
public sealed class AssessmentRunRepository(AtlasDbContext db) : IAssessmentRunRepository
{
    ...
    public async Task<IReadOnlyList<Atlas.Application.Portfolio.CompletedRunPoint>> ListCompletedPointsAsync(string? tag, CancellationToken cancellationToken)
    {
        ...
        // Tag matching is done on the deserialized list: JSON string containment would miss non-ASCII
        // tags (System.Text.Json escapes them) and conflate tags containing quotes.
        HashSet<Guid>? taggedIds = null;
        if (tag is not null)
        {
            var wanted = tag.Trim();
            taggedIds = (await db.Assessments
                    .Where(a => a.TagsJson != null)
                    .Select(a => new { a.Id, a.TagsJson })
                    .ToListAsync(cancellationToken))
                .Where(a => (System.Text.Json.JsonSerializer.Deserialize<List<string>>(a.TagsJson!) ?? [])
        ...
        var rows = await db.HealthSnapshots
            .Where(h => h.CreatedAtUtc >= cutoff && db.Assessments.Any(a => a.Id == h.AssessmentId)
                && (taggedIds == null || taggedIds.Contains(h.AssessmentId)))
...
public sealed class FindingRepository(AtlasDbContext db) : IFindingRepository
{
    ...
    public async Task<IReadOnlyList<Finding>> ListWithPredecessorAsync(Guid assessmentId, CancellationToken cancellationToken) =>
        await db.Findings
            .Where(f => f.AssessmentId == assessmentId
                && f.PredecessorFingerprint != null
                && (f.Status == FindingStatus.Open || f.Status == FindingStatus.Regressed))
            .ToListAsync(cancellationToken);

    public async Task<IReadOnlyDictionary<Guid, string?>> GetLatestOccurrenceDataByRuleAsync(
        IReadOnlyCollection<Guid> assessmentIds, string ruleId, CancellationToken cancellationToken)
    {
        var heads = await db.Findings
            .Where(f => assessmentIds.Contains(f.AssessmentId) && f.RuleId == ruleId && (f.Status == FindingStatus.Open || f.Status == FindingStatus.Regressed))
            .Select(f => new { f.Id, f.AssessmentId })
            .ToListAsync(cancellationToken);
        if (heads.Count == 0)
        {
        ...
        var findingIds = heads.Select(h => h.Id).ToList();
        var latest = (await db.FindingOccurrences
                .Where(o => findingIds.Contains(o.FindingId))
                .OrderByDescending(o => o.CreatedAtUtc)
                .Select(o => new { o.FindingId, o.DataJson, o.CreatedAtUtc })
                .ToListAsync(cancellationToken))
            .GroupBy(o => o.FindingId)
            .ToDictionary(g => g.Key, g => g.First().DataJson);

        // One inventory finding per assessment by construction; if history ever holds two, the newest occurrence wins.
        return heads
            .GroupBy(h => h.AssessmentId)
            .ToDictionary(g => g.Key, g => g.Select(h => latest.GetValueOrDefault(h.Id)).FirstOrDefault(d => d is not null));
    }
...
public sealed class RuleCatalog(AtlasDbContext db) : IRuleCatalog
{
        ...
        string scannerId, IReadOnlyList<RuleSpec> rules, CancellationToken cancellationToken)
    {
        ...
        // Idempotent under concurrency: the catalog is global and keyed by rule id, so two workers
        // (or two tenants) running a brand-new scanner at once both miss the read and both try to
        // insert the same ids. Without this retry the unique-violation would fail the whole scan on
        // exactly the "new language just shipped, all ids are new" rollout. On conflict we discard
        // our pending rule changes, re-read (now seeing the row the other writer created), and retry.
        for (var attempt = 0; ; attempt++)
        {
            ...
            {
                // Only rule changes are pending here (findings are added after this returns), so it
                // is safe to detach them all and rebuild from a fresh read on the retry.
                foreach (var entry in db.ChangeTracker.Entries<RuleDefinition>()
    ...
    public async Task DeleteScannerRulesAsync(string scannerId, CancellationToken cancellationToken)
    {
        var rules = await db.RuleDefinitions.Where(r => r.ScannerId == scannerId).ToListAsync(cancellationToken);
        db.RuleDefinitions.RemoveRange(rules);
    }
...
public sealed class HealthRepository(AtlasDbContext db) : IHealthRepository
{
    ...
    public async Task<IReadOnlyDictionary<Guid, HealthSnapshot>> GetLatestForAsync(
        IReadOnlyCollection<Guid> assessmentIds, CancellationToken cancellationToken)
    {
        // Latest snapshot per assessment WITHOUT materializing every snapshot ever written: keep
        // only rows that have no newer sibling (NOT EXISTS → correlated subquery over the
        // {AssessmentId, CreatedAtUtc} index). The portfolio used to load the full history of up to
        // 500 assessments just to discard all but the newest of each.
        var latest = await db.HealthSnapshots
            .Where(h => assessmentIds.Contains(h.AssessmentId))
            .Where(h => !db.HealthSnapshots.Any(o => o.AssessmentId == h.AssessmentId && o.CreatedAtUtc > h.CreatedAtUtc))
            .ToListAsync(cancellationToken);

        // A theoretical CreatedAtUtc tie could return two rows; take one deterministically.
        return latest
            .GroupBy(h => h.AssessmentId)
...
public sealed class InventoryRepository(AtlasDbContext db) : IInventoryRepository
{
    ...
    public async Task<IReadOnlyDictionary<Guid, IReadOnlyList<InventorySnapshot>>> GetLatestForAsync(
        IReadOnlyCollection<Guid> assessmentIds, CancellationToken cancellationToken)
    {
        // Latest snapshot per (assessment, language) via NOT EXISTS — no full-history materialization.
        var latest = await db.InventorySnapshots
            .Where(s => assessmentIds.Contains(s.AssessmentId))
            .Where(s => !db.InventorySnapshots.Any(o => o.AssessmentId == s.AssessmentId && o.LanguageId == s.LanguageId && o.CreatedAtUtc > s.CreatedAtUtc))
            .ToListAsync(cancellationToken);
...
public sealed class TenantAiEstateSettingsRepository(AtlasDbContext db) : Atlas.Application.AiEstate.ITenantAiEstateSettingsRepository
{
    public Task<Atlas.Domain.AiEstate.TenantAiEstateSettings?> GetForTenantAsync(Guid tenantId, CancellationToken cancellationToken) =>
        db.TenantAiEstateSettings.IgnoreQueryFilters().SingleOrDefaultAsync(s => s.TenantId == tenantId, cancellationToken);

    public void Add(Atlas.Domain.AiEstate.TenantAiEstateSettings settings) => db.TenantAiEstateSettings.Add(settings);

    public void Remove(Atlas.Domain.AiEstate.TenantAiEstateSettings settings) => db.TenantAiEstateSettings.Remove(settings);
}
