...
public sealed class AtlasDbContext(DbContextOptions<AtlasDbContext> options, Atlas.Application.Tenants.ITenantContext? tenant = null) : DbContext(options)
{
    ...
    public DbSet<Atlas.Domain.Modernization.TenantCostProfile> TenantCostProfiles => Set<Atlas.Domain.Modernization.TenantCostProfile>();

    public DbSet<TenantNotificationSettings> TenantNotificationSettings => Set<TenantNotificationSettings>();

    public DbSet<Atlas.Domain.AiEstate.TenantAiEstateSettings> TenantAiEstateSettings => Set<Atlas.Domain.AiEstate.TenantAiEstateSettings>();

    public DbSet<Atlas.Domain.Operations.SystemMarker> SystemMarkers => Set<Atlas.Domain.Operations.SystemMarker>();
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Finding>(entity =>
        {
            ...
            entity.Property(f => f.PredecessorFingerprint).HasMaxLength(64);
        ...
        modelBuilder.Entity<Atlas.Domain.Operations.SystemMarker>(entity =>
        {
            entity.ToTable("system_markers");
            entity.HasKey(m => m.Key);
            entity.Property(m => m.Key).HasMaxLength(200);
            entity.Property(m => m.Value).HasMaxLength(2000);
        });

        modelBuilder.Entity<TenantNotificationSettings>(entity =>
        {
            entity.ToTable("tenant_notification_settings");
            entity.HasKey(sNotif => sNotif.TenantId);
            entity.Property(sNotif => sNotif.WebhookUrl).HasMaxLength(1000);
            entity.Property(sNotif => sNotif.Secret).HasMaxLength(500);
            entity.Property(sNotif => sNotif.SlackWebhookUrl).HasMaxLength(1000);
            entity.Property(sNotif => sNotif.TeamsWebhookUrl).HasMaxLength(1000);
            entity.Property(sNotif => sNotif.DigestDayOfWeek).HasMaxLength(16);
            entity.Property(sNotif => sNotif.UpdatedBy).HasMaxLength(200);
        });

        modelBuilder.Entity<Atlas.Domain.Modernization.TenantCostProfile>(entity =>
        {
            entity.ToTable("tenant_cost_profiles");
            entity.HasKey(p => p.TenantId);
            entity.Property(p => p.Currency).HasMaxLength(3);
            entity.Property(p => p.UpdatedBy).HasMaxLength(200);
        });

        modelBuilder.Entity<Atlas.Domain.AiEstate.TenantAiEstateSettings>(entity =>
        {
            entity.ToTable("tenant_ai_estate_settings");
            entity.HasKey(s => s.TenantId);
            entity.Property(s => s.ApprovedProvidersJson).HasColumnType("jsonb").IsRequired();
            entity.Property(s => s.UpdatedBy).HasMaxLength(200).IsRequired();
            entity.Ignore(s => s.ApprovedProviders);
            entity.Ignore(s => s.HasAllowlist);
        });
    ...
    private void ApplyTenantFilters(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Atlas.Domain.Modernization.TenantCostProfile>().HasQueryFilter(e => _tenant.TenantId == null || e.TenantId == _tenant.TenantId);
        modelBuilder.Entity<TenantNotificationSettings>().HasQueryFilter(e => _tenant.TenantId == null || e.TenantId == _tenant.TenantId);
        modelBuilder.Entity<Atlas.Domain.AiEstate.TenantAiEstateSettings>().HasQueryFilter(e => _tenant.TenantId == null || e.TenantId == _tenant.TenantId);
