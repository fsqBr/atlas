...
public sealed class SuppressionExpiryService(IServiceScopeFactory scopeFactory, ILogger<SuppressionExpiryService> logger) : BackgroundService
{
    ...
    private async Task SweepAsync(CancellationToken cancellationToken)
    {
        ...
        var activeRows = await suppressions.GetActiveForAsync(expired.Select(f => f.Id).ToList(), cancellationToken);
        foreach (var finding in expired)
        {
            // Revoke the spent waiver so the audit trail never shows an "active" suppression on an
            // Open finding, then reopen.
            if (activeRows.TryGetValue(finding.Id, out var waiver))
            {
                waiver.Revoke("system:waiver-expired");
            }
