...
public static class AuthSetup
{
                ...
                jwt.Events = new JwtBearerEvents
                {
                    OnMessageReceived = context =>
                    {
                        ...
                        if (string.IsNullOrEmpty(context.Token)
                            && (path.EndsWith("/report", StringComparison.Ordinal) || path.EndsWith("/report.pdf", StringComparison.Ordinal) || path.EndsWith("/findings/export", StringComparison.Ordinal) || path.StartsWith("/api/events/", StringComparison.Ordinal) || path.EndsWith("/compliance.zip", StringComparison.Ordinal))
    ...
    public static void UseAtlasAuth(this WebApplication app, AuthOptions options)
    {
        ...
        app.Use(async (context, next) =>
        {
            ...
            if (protectedRoute && RequiresRoleCheck(context.Request.Method, path.Value ?? string.Empty))
            {
    ...
    /// <summary>
    /// Reads are open to every authenticated user (the viewer role), writes go through <see cref="RequiredRole"/> —
    /// except the administrative inventories (API tokens, stored credentials, tenants), whose listings are admin-only
    /// even for GET: token names/hints and credential names are operational detail nobody else needs.
    /// </summary>
    internal static bool RequiresRoleCheck(string method, string path)
    {
        ...
        return path.StartsWith("/api/tokens", StringComparison.OrdinalIgnoreCase)
            || path.StartsWith("/api/credentials", StringComparison.OrdinalIgnoreCase)
    ...
    {
        if (path.StartsWith("/api/ai-estate/usage/report", StringComparison.OrdinalIgnoreCase) || path.StartsWith("/api/ai-estate/otlp/", StringComparison.OrdinalIgnoreCase))
        {
            return options.AnalystRole; // a developer reporting their own usage through the opt-in CLI
        }
        ...
        var admin = path.StartsWith("/api/credentials", StringComparison.OrdinalIgnoreCase)
            ...
            || (path.StartsWith("/api/ai-estate", StringComparison.OrdinalIgnoreCase) && !HttpMethods.IsGet(method))
            ...
            || (path.StartsWith("/api/settings/notifications", StringComparison.OrdinalIgnoreCase) && !HttpMethods.IsGet(method))
            || path.StartsWith("/api/tenants", StringComparison.OrdinalIgnoreCase)
