...
internal static class ApiMapping
{
    ...
    public static AssessmentResponse ToResponse(Assessment a, IReadOnlyList<Scan> scans, ScanJobState? activeJob) => new(
        ...
        scans.Select(ToResponse).ToList(), activeJob?.ToString(), a.Tags);
    ...
    public static ModernizationPlanResponse ToResponse(ModernizationPlan plan, string? lang)
    {
        ...
        var strategies = plan.Assessment.Strategies
            ...
            .Select(s =>
            {
                ...
                return new StrategyResponse(
                    ...
                    ToResponse(estimate, lang),
                    plan.PaybackMonths(estimate));
            })
            .ToList();
        ...
        var savings = plan.Savings is null ? null : new SavingsResponse(
            plan.Savings.ModelVersion,
            plan.Savings.Items.Select(i => new SavingsItemResponse(i.Key, ModernizationTexts.Text(i.Key, lang), i.AnnualAmount, i.Quantity)).ToList(),
            plan.Savings.AnnualTotal,
            plan.Savings.Currency,
            plan.Savings.Assumptions.Select(a => new AssumptionResponse(a.Key, ModernizationTexts.Text(a.Key, lang), a.Value)).ToList());

        return new ModernizationPlanResponse(
            plan.Assessment.ModelVersion, profile, plan.Assessment.Recommended.ToString(),
            ModernizationTexts.Strategy(plan.Assessment.Recommended, lang), strategies, roadmap, savings);
    }
    ...
    public static PortfolioResponse ToResponse(PortfolioSummary p) => new(
        ...
        p.Rows.Select(r => new PortfolioRowResponse(r.Id, r.Name, r.SourceKind, r.Status, r.Score, r.Risk?.ToString(), r.OpenFindings, r.Lines, r.Projects, r.LegacyProjects, r.CompletedAtUtc, r.Percentile, r.TargetScore, r.TargetDate, r.TargetStatus.ToString(), r.Tags)).ToList(),
        ...
        p.AiEstate is null ? null : ToResponse(p.AiEstate));

    public static PortfolioAiEstateResponse ToResponse(Atlas.Application.AiEstate.PortfolioAiEstate e) => new(
        e.AssessmentsWithAi, e.AssessmentsScanned,
        e.Providers.Select(p => new PortfolioAiProviderResponse(p.Id, p.Name, p.Kind, p.Assessments, p.Approved)).ToList(),
        e.Frameworks.Select(f => new PortfolioFrameworkCountResponse(f.Framework, f.Count)).ToList(),
        e.McpServers, e.RemoteMcpServers, e.SecretsInMcp, e.RetiredModelReferences, e.AllowlistConfigured,
        e.Unapproved.Select(u => new PortfolioFrameworkCountResponse(u.Provider, u.Assessments)).ToList(),
        e.AiWithPii, e.AiWithSecrets,
        e.Rows.Select(r => new PortfolioAiRowResponse(r.AssessmentId, r.Name, r.Providers, r.Frameworks, r.McpServers, r.Unapproved, r.RetiredModels, r.SecretsInMcp, r.PiiCoLocated, r.SecretsCoLocated, r.Tags)).ToList(),
        e.CatalogVersion,
        e.Costs is null ? null : ToResponse(e.Costs));

    public static AssessmentAiEstateResponse ToResponse(Atlas.Application.AiEstate.AssessmentAiEstate e) => new(
        e.Scanned,
        e.Record is null ? null : new AiEstateRecordResponse(
            e.Record.CatalogVersion, e.Record.CatalogHash,
            e.Record.Providers.Select(p => new AiEstateProviderResponse(p.Id, p.Name, p.Kind, p.Packages, p.EndpointFiles, p.EnvVars, p.Models, p.CodeFiles, p.Approved)).ToList(),
            e.Record.Frameworks.Select(f => new AiEstateFrameworkResponse(f.Name, f.Packages)).ToList(),
            e.Record.Mcp.Select(m => new AiEstateMcpServerResponse(m.Config, m.Name, m.Transport, m.Capability, m.Remote, m.Host, m.Command, m.Known, m.Secrets)).ToList(),
            e.Record.ActiveModels,
            e.Record.RetiredModels.Select(r => new AiEstateRetiredModelResponse(r.Model, r.RetiredOn, r.Replacement, r.Files)).ToList(),
            e.Record.VectorStores, e.Record.LocalRuntimes, e.Record.Gateways,
            e.Record.AllowlistConfigured, e.Record.Approved, e.Record.Unapproved, e.Record.SecretsInMcp),
        e.OpenPii, e.OpenSecrets,
        e.AiRules.Select(r => new AssessmentAiRuleResponse(r.RuleId, r.Title, r.MaxSeverity.ToString(), r.Count)).ToList());

    public static AiCostSummaryResponse ToResponse(Atlas.Application.AiEstate.AiCostSummary c) => new(
        c.Days, c.From, c.To,
        c.Providers.Select(p => new AiCostProviderResponse(p.Provider, p.Basis, p.Currency, p.Total, p.TopDimensions.Select(d => new AiCostDimensionResponse(d.Key, d.Amount)).ToList(), p.LastSyncAtUtc, p.LastSyncStatus, p.LastSyncError)).ToList(),
        c.Seats.Select(s => new AiSeatSummaryResponse(s.Provider, s.Total, s.ActiveWithinIdleWindow, s.Idle, s.PendingCancellation, s.Plan)).ToList(),
        c.Activity.Select(a => new AiActivityResponse(a.Provider, a.Key, a.AveragePerDay, a.Unit)).ToList(),
        c.SourcesConfigured);

    public static SuppressionPolicyResponse ToResponse(SuppressionPolicy p) => new(p.Id, p.AssessmentId, p.RulePattern, p.PathGlob, p.Reason, p.Author, p.CreatedAtUtc, p.ExpiresAtUtc);
