...
assessments.MapPut("/{id:guid}/actuals", async (
    Guid id, RecordActualRequest request, IAssessmentRepository assessmentsRepo, IModernizationActualRepository repository, IUnitOfWork unitOfWork, CancellationToken ct, string? lang = null) =>
{
    ...
    try
    {
        ...
        if (existing is null)
        {
            existing = new Atlas.Domain.Modernization.ModernizationActual(id, assessment.TenantId, strategy, request.ActualHours, request.ActualMonths, request.ActualCost, request.Currency ?? "BRL", request.Notes, request.RecordedBy);
            repository.Add(existing);
        }
        else
        {
            existing.Update(strategy, request.ActualHours, request.ActualMonths, request.ActualCost, request.Currency ?? existing.Currency, request.Notes, request.RecordedBy);
        }
    ...
    string? lang = null) =>
{
    ...
    var report = await reportBuilder.BuildAsync(id, locale, ct);
