    ...
    CancellationToken ct) =>
{
    ...
    if (httpRequest.ContentLength is > 25_000_000)
    {
    ...
    runsRepo.Add(run);
    var toolVersion = import.ToolVersion.Length > 50 ? import.ToolVersion[..50] : import.ToolVersion;
    var scan = Atlas.Domain.Scans.Scan.Start(Guid.NewGuid(), assessment.TenantId, id, Guid.Empty, import.ScannerId, toolVersion, null, run.Id);
    scansRepo.Add(scan);
    ...
    var reconciliation = Atlas.Application.Findings.FindingReconciler.Reconcile(
        assessment.TenantId, id, scan.Id, import.ScannerId, toolVersion,
        assessment.RepositoryKey, import.Candidates, rules, existing, scanSucceeded: true);

    findingsRepo.AddRange(reconciliation.Created);
    findingsRepo.AddOccurrences(reconciliation.Occurrences);
    scan.Succeed(import.Candidates.Count, reconciliation.Created.Count, reconciliation.Recurring, reconciliation.Resolved, reconciliation.Regressed);
...
app.MapGet("/api/settings/cost", async (ITenantCostProfileRepository profiles, Atlas.Domain.Modernization.CostParameters defaults, CancellationToken ct) =>
{
...
app.MapPut("/api/settings/cost", async (CostProfileRequest request, ITenantCostProfileRepository profiles, Atlas.Application.Tenants.ITenantContext tenant, IUnitOfWork unitOfWork, CancellationToken ct) =>
{
    try
    {
        var author = string.IsNullOrWhiteSpace(request.Author) ? tenant.SubjectName ?? tenant.Subject ?? "unknown" : request.Author;
        ...
        await unitOfWork.SaveChangesAsync(ct);
        return Results.Ok(new CostProfileResponse(profile.Currency, profile.HourlyRate, profile.TeamSize ?? 0, IsDefault: false, profile.UpdatedBy, profile.UpdatedAtUtc));
    }
    catch (ArgumentException ex)
    {
        return Results.BadRequest(new { error = ex.Message });
...
app.MapDelete("/api/settings/cost", async (ITenantCostProfileRepository profiles, IUnitOfWork unitOfWork, CancellationToken ct) =>
{
    var profile = await profiles.GetAsync(ct);
