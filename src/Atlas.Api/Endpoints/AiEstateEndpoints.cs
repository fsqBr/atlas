...
internal static class AiEstateEndpoints
{
    ...
    private static TeamResponse TeamRow(Team t) => new(t.Id, t.Name, t.Members, t.UpdatedBy, t.UpdatedAtUtc);
    ...
    public static void Map(WebApplication app)
    {
        ...
        group.MapPut("/teams", async (UpsertTeamRequest request, TeamService teams, CancellationToken ct) =>
        {
            try
            {
                return Results.Ok(TeamRow(await teams.UpsertAsync(request.Id, request.Name, request.Members ?? [], request.Author, ct)));
            }
            catch (ArgumentException ex)
            {
                return Results.BadRequest(new { error = ex.Message });
