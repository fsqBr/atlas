    ...
    ILogger<WeeklyDigestService> logger) : BackgroundService
{
    ...
    private async Task SendAsync(DateTimeOffset now, CancellationToken cancellationToken)
    {
        ...
        var all = await assessments.ListRecentAsync(2000, cancellationToken);
        // The worker runs without a tenant context, and the chat webhooks are one per deployment:
        // in a multi-tenant install a combined digest would leak one tenant's assessment names and
        // scores into another tenant's channel — refuse instead.
        if (all.Select(a => a.TenantId).Distinct().Count() > 1)
        {
            logger.LogWarning("Weekly digest skipped: this deployment has multiple tenants and the chat webhooks are deployment-wide. Per-tenant digests are not supported yet.");
            return;
        }
