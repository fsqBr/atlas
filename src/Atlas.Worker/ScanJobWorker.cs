    ...
    ILogger<ScanJobWorker> logger) : BackgroundService
{
    ...
    {
        await SchemaGate.WaitAsync(scopeFactory, logger, stoppingToken);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                // First boot: the API applies the migrations; until then the tables do not exist. Not an error.
                logger.LogInformation("Schema not ready yet ({Relation}); the API applies migrations at start. Retrying.", ex.MessageText);
            }
            catch (Exception ex)
            {
