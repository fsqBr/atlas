...
public sealed class NotificationOptions
{
    ...
    /// <summary>Day of week ("Monday") for the weekly portfolio digest on the chat webhooks; null/empty = off.</summary>
    public string? DigestDayOfWeek { get; set; }

    /// <summary>UTC hour (0–23) the weekly digest fires.</summary>
    public int DigestHourUtc { get; set; } = 13;
}

/// <summary>A tenant's channel overrides, resolved by the worker from tenant_notification_settings.</summary>
public sealed record TenantNotificationOverrides(string? WebhookUrl, string? Secret, string? SlackWebhookUrl, string? TeamsWebhookUrl);
...
public sealed class RunNotifier(IHttpClientFactory httpClientFactory, NotificationOptions options, ILogger<RunNotifier> logger)
{
    ...
    public async Task NotifyAsync(RunCompletedPayload payload, string? assessmentWebhook, CancellationToken cancellationToken, TenantNotificationOverrides? overrides = null)
    {
        // A tenant with its own settings row is ISOLATED: its runs never reach the deployment-wide
        // channels (per-field fallback would leak one tenant's runs into the shared Slack/webhook).
        var slack = overrides is null ? options.SlackWebhookUrl : overrides.SlackWebhookUrl;
        var teams = overrides is null ? options.TeamsWebhookUrl : overrides.TeamsWebhookUrl;
        var tenantWebhook = overrides is null ? options.WebhookUrl : overrides.WebhookUrl;
        var signingSecret = overrides is null ? options.Secret : overrides.Secret;

        await PostChatAsync(slack, ChatNotifications.Slack(payload), "Slack", payload, cancellationToken);
        await PostChatAsync(teams, ChatNotifications.Teams(payload), "Teams", payload, cancellationToken);
        var targets = new[] { assessmentWebhook, tenantWebhook }
        ...
        foreach (var target in targets)
        {
            try
            {
                ...
                {
                    request.Headers.Add("X-Atlas-Signature", "sha256=" + Sign(body, signingSecret));
                }
