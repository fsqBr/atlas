...
public static partial class LicenseClassifier
{
    ...
    [
        ...
        ("go.microsoft.com/fwlink/?linkid=262998", "MS-NET-Library"), ("microsoft.com/web/webpi/eula", "MS-NET-Library"), ("go.microsoft.com/fwlink", "MS-NET-Library"),
    ...
    public static LicenseClass Classify(string? expression)
    {
        ...
        {
            return text.Split(" OR ", StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).Select(Classify).Where(c => c != LicenseClass.Unknown).DefaultIfEmpty(LicenseClass.Unknown).Min();
        }

        if (text.Contains(" AND ", StringComparison.OrdinalIgnoreCase))
        {
            return text.Split(" AND ", StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).Select(Classify).Max();
        }
