...
public sealed record TriageRequest(string Action, string? Reason, string? Author, DateTimeOffset? ExpiresAtUtc = null);
...
public sealed record ModernizationPlanResponse(
    ...
    SavingsResponse? Savings = null);

public sealed record SavingsItemResponse(string Key, string Label, decimal AnnualAmount, double Quantity);

public sealed record SavingsResponse(
    string ModelVersion,
    IReadOnlyList<SavingsItemResponse> Items,
    decimal AnnualTotal,
    string Currency,
    IReadOnlyList<AssumptionResponse> Assumptions);

public sealed record IssueExportRequest(int? Top = null);

public sealed record IssueExportResponse(int Created, IReadOnlyList<string> Urls, IReadOnlyList<string> Errors);
...
public sealed record PortfolioResponse(
    ...
    PortfolioAiEstateResponse? AiEstate = null);

// ---- AI Estate across the portfolio (V0.5 governance module; team attribution by tags only) ----

public sealed record PortfolioAiProviderResponse(string Id, string Name, string Kind, int Assessments, bool? Approved);

public sealed record PortfolioAiRowResponse(
    Guid AssessmentId, string Name, IReadOnlyList<string> Providers, IReadOnlyList<string> Frameworks,
    int McpServers, int Unapproved, int RetiredModels, int SecretsInMcp, bool PiiCoLocated, bool SecretsCoLocated, IReadOnlyList<string>? Tags);

public sealed record PortfolioAiEstateResponse(
    int AssessmentsWithAi,
    int AssessmentsScanned,
    IReadOnlyList<PortfolioAiProviderResponse> Providers,
    IReadOnlyList<PortfolioFrameworkCountResponse> Frameworks,
    int McpServers,
    int RemoteMcpServers,
    int SecretsInMcp,
    int RetiredModelReferences,
    bool AllowlistConfigured,
    IReadOnlyList<PortfolioFrameworkCountResponse> Unapproved,
    int AiWithPii,
    int AiWithSecrets,
    IReadOnlyList<PortfolioAiRowResponse> Rows,
    string? CatalogVersion,
    AiCostSummaryResponse? Costs = null);

public sealed record PortfolioFrameworkCountResponse(string Name, int Count);

// ---- AI estate of one assessment (the UI tab; same facts as the report section) ----

public sealed record AiEstateProviderResponse(string Id, string Name, string Kind, IReadOnlyList<string> Packages, int EndpointFiles, IReadOnlyList<string> EnvVars, IReadOnlyList<string> Models, int CodeFiles, bool? Approved);

public sealed record AiEstateFrameworkResponse(string Name, IReadOnlyList<string> Packages);

public sealed record AiEstateMcpServerResponse(string Config, string Name, string Transport, string Capability, bool Remote, string? Host, string? Command, string? Known, int Secrets);

public sealed record AiEstateRetiredModelResponse(string Model, string? RetiredOn, string? Replacement, int Files);

public sealed record AiEstateRecordResponse(
    string CatalogVersion,
    string CatalogHash,
    IReadOnlyList<AiEstateProviderResponse> Providers,
    IReadOnlyList<AiEstateFrameworkResponse> Frameworks,
    IReadOnlyList<AiEstateMcpServerResponse> Mcp,
    IReadOnlyList<string> ActiveModels,
    IReadOnlyList<AiEstateRetiredModelResponse> RetiredModels,
...
public sealed record AssessmentAiRuleResponse(string RuleId, string Title, string MaxSeverity, int Count);

/// <summary>The tenant's approved AI providers. Source: tenant (saved in the UI), config (Atlas:AiEstate:ApprovedProviders) or none.</summary>
public sealed record AiEstateAllowlistResponse(IReadOnlyList<string> ApprovedProviders, string Source, string? UpdatedBy, DateTimeOffset? UpdatedAtUtc);

public sealed record AiEstateAllowlistRequest(IReadOnlyList<string> ApprovedProviders, string? Author = null);

public sealed record AiCatalogProviderResponse(string Id, string Name, string Kind);

public sealed record AssessmentAiEstateResponse(bool Scanned, AiEstateRecordResponse? Record, int OpenPii, int OpenSecrets, IReadOnlyList<AssessmentAiRuleResponse> AiRules);

// ---- AI spend (governance module; bases are reported separately and never summed) ----

public sealed record AiCostDimensionResponse(string Key, decimal Amount);

public sealed record AiCostProviderResponse(
    string Provider, string Basis, string Currency, decimal Total, IReadOnlyList<AiCostDimensionResponse> TopDimensions,
    DateTimeOffset? LastSyncAtUtc, string? LastSyncStatus, string? LastSyncError);

public sealed record AiSeatSummaryResponse(string Provider, int Total, int Active, int Idle, int PendingCancellation, string? Plan);

public sealed record AiActivityResponse(string Provider, string Key, decimal AveragePerDay, string Unit);

public sealed record AiCostSummaryResponse(
    int Days, DateOnly From, DateOnly To,
    IReadOnlyList<AiCostProviderResponse> Providers,
    IReadOnlyList<AiSeatSummaryResponse> Seats,
    IReadOnlyList<AiActivityResponse> Activity,
    int SourcesConfigured);

// ---- Suppression policies ----

public sealed record SuppressionPolicyResponse(Guid Id, Guid? AssessmentId, string RulePattern, string? PathGlob, string Reason, string Author, DateTimeOffset CreatedAtUtc, DateTimeOffset? ExpiresAtUtc = null);

public sealed record CreatePolicyRequest(string RulePattern, string? PathGlob, string Reason, string Author, DateTimeOffset? ExpiresAtUtc = null);

// ---- Waiver migration after a rule major bump ----

public sealed record MigratableWaiverResponse(
    Guid FindingId,
    string RuleId,
    string Title,
    string Severity,
    ...
    string WaiverReason,
    string WaiverAuthor,
    DateTimeOffset WaiverCreatedAtUtc,
    DateTimeOffset? WaiverExpiresAtUtc);

/// <summary>Null or empty FindingIds migrates every migratable waiver of the assessment.</summary>
public sealed record MigrateWaiversRequest(IReadOnlyList<Guid>? FindingIds, string Author);

public sealed record WaiverMigrationResponse(int Migrated, int Skipped, IReadOnlyList<Guid> MigratedFindingIds);

public sealed record TagsRequest(IReadOnlyList<string> Tags);

// ---- Tenant cost profile (Settings → Cost) ----

public sealed record CostProfileResponse(string Currency, decimal HourlyRate, int TeamSize, bool IsDefault, string? UpdatedBy, DateTimeOffset? UpdatedAtUtc, decimal? WindowsHostingPerLegacyAppYear = null, decimal? ExtendedSupportPerLegacyAppYear = null, decimal? SqlServerSavingsPerYear = null);

public sealed record CostProfileRequest(string Currency, decimal HourlyRate, int? TeamSize, string? Author, decimal? WindowsHostingPerLegacyAppYear = null, decimal? ExtendedSupportPerLegacyAppYear = null, decimal? SqlServerSavingsPerYear = null);

// ---- Tenant notification settings (Settings → Administration) ----
...
public sealed record QualityGateResponse(bool Passed, bool Evaluated, int? Score, IReadOnlyDictionary<string, int> OpenBySeverity, IReadOnlyList<string> Violations, string? FailOn, int? MinScore, string? ReportUrl, string? FailOnNew = null, IReadOnlyDictionary<string, int>? NewBySeverity = null);
