...
public sealed record CostProfileResponse(string Currency, decimal HourlyRate, int TeamSize, bool IsDefault, string? UpdatedBy, DateTimeOffset? UpdatedAtUtc);

public sealed record CostProfileRequest(string Currency, decimal HourlyRate, int? TeamSize, string? Author);
