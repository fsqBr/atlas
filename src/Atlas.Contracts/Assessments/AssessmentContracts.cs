...
public sealed record PortfolioTrendPointResponse(DateOnly Date, double? AverageScore, int OpenFindings, int Assessed);
...
public sealed record RecordActualRequest(string Strategy, double ActualHours, double? ActualMonths, decimal? ActualCost, string? Currency, string? Notes, string RecordedBy);
