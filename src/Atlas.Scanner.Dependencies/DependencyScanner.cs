...
public sealed class DependencyScanner(IVulnerabilitySource vulnerabilities) : IScanner
{
    ...
    /// <summary>Label severities map directly; CVSS vector strings are scored (a 9.8 vector was landing
    /// on Medium); anything unreadable stays a conservative Medium.</summary>
    private static Severity MapOsvSeverity(string? severity) =>
        severity?.Trim().ToUpperInvariant() switch
        {
            ...
            "LOW" => Severity.Low,
            { } vector when vector.StartsWith("CVSS:", StringComparison.Ordinal) =>
                Vulnerabilities.CvssVector.ToSeverity(vector) ?? Severity.Medium,
            _ => Severity.Medium,
        };
