...
public sealed class PriceCatalog
{
    ...
    /// <summary>A catalog where <paramref name="overrides"/> are consulted first, then this catalog's entries.</summary>
    public PriceCatalog WithOverrides(IEnumerable<ModelPrice> overrides, string version)
    {
        var models = overrides.Concat(Document.Models).ToList();
        return new PriceCatalog(Document with { Version = version, Models = models }, Source);
    }
