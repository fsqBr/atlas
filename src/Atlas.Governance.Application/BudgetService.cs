...
public interface IAlertSink
{
    ...
    Task<string?> SendAsync(Guid tenantId, string title, string body, CancellationToken cancellationToken);
}

public sealed class NullAlertSink : IAlertSink
{
    public Task<string?> SendAsync(Guid tenantId, string title, string body, CancellationToken cancellationToken) => Task.FromResult<string?>("no notification channel configured");
}
    ...
    ILogger<BudgetService> logger)
{
    ...
    public async Task<int> EvaluateAsync(CancellationToken ct)
    {
        ...
        {
            var error = await sink.SendAsync(tenantId, alert.Kind == "anomaly" ? "Atlas: unusual AI spend" : "Atlas: AI budget", alert.Message, ct);
...
public sealed class TeamService(ITeamRepository teams, IUsageFactRepository facts, IGovernanceUnitOfWork unitOfWork, ITenantContext tenant)
{
    ...
    public async Task<Team> UpsertAsync(Guid? id, string name, IEnumerable<string> members, string? updatedBy, CancellationToken ct)
    {
