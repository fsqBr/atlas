...
public sealed class TelemetryOptions
{
    ...
    /// <summary>pseudonym (default): people appear as id-…, an HMAC of the identity attribute (shared with the Cursor connector). label: the attribute itself (e-mail, id).</summary>
    ...
    ILogger<TelemetryIngestService> logger)
{
    ...
    private string Actor(TelemetrySample s, byte[]? hmacKey)
    {
        ...
        return "id-" + HmacFingerprint.Compute(hmacKey, s.RawActor.Trim().ToLowerInvariant())[..12];
    }
