...
public sealed record UsageActorSummary(string Actor, IReadOnlyList<string> Tools, int Sessions, int Requests, long Tokens, decimal EstimatedCost, long UnpricedTokens, DateTimeOffset LastReportUtc, DateOnly LastPeriod);
    ...
    ILogger<UsageReportService> logger)
{
    ...
    public async Task<AiUsageSummary?> SummaryAsync(int days, CancellationToken cancellationToken)
    {
        ...
        var rows = await facts.ListAsync(from, to, cancellationToken);
        return rows.Count == 0 ? null : Build(days, from, to, rows, prices);
    }

    internal static AiUsageSummary Build(int days, DateOnly from, DateOnly to, IReadOnlyList<UsageFact> rows, PriceCatalog prices)
    {
        var actors = rows
