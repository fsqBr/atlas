    ...
    IUsageEventRepository? events = null)
{
    ...
    public async Task<AiUsageSummary?> SummaryAsync(int days, CancellationToken cancellationToken)
    {
        ...
        var prices = await priceResolver.ResolveAsync(cancellationToken);
        return Build(days, from, to, rows, prices, all);
    }
    ...
    internal static AiUsageSummary Build(int days, DateOnly from, DateOnly to, IReadOnlyList<UsageFact> rows, PriceCatalog prices, IReadOnlyList<UsageFact>? history = null)
    {
        history ??= rows;
        var actors = rows
