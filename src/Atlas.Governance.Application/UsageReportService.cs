...
public sealed class UsageReportService(
    ...
    ICostFactRepository? costs = null)
{
    ...
    public async Task<AiUsageSummary?> SummaryAsync(int days, CancellationToken cancellationToken)
    {
        ...
        {
            var costRows = await costs.ListAsync(loadFrom, to, cancellationToken);
            ratio = Reconciliations.Build(all, costRows, loadFrom, to).BlendedRatio;
        }

        return Build(days, from, to, rows, prices, all, teamList, ratio);
    }
    ...
    internal static AiUsageSummary Build(int days, DateOnly from, DateOnly to, IReadOnlyList<UsageFact> rows, PriceCatalog prices, IReadOnlyList<UsageFact>? history = null, IReadOnlyList<Team>? teamList = null, decimal? calibrationRatio = null)
    {
        history ??= rows;
        teamList ??= [];
        var monthStart = new DateOnly(to.Year, to.Month, 1);
        var actors = rows
            ...
            .Select(g => new UsageActorSummary(
                ...
                g.Max(r => r.Period),
                UsageForecasts.Build(history.Where(h => h.Actor == g.Key), to, calibrationRatio)))
        ...
        return new AiUsageSummary(
            ...
            UsageForecasts.Build(history, to, calibrationRatio),
