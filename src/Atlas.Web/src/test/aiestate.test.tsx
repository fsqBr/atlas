...
import { BurnUp, CliGuide, ForecastTiles, LiveView, UsageView } from "../pages/DeveloperUsageCard";
import { ReconciliationView } from "../pages/ReconciliationCard";
...
import { AlertsList, BudgetsView } from "../pages/BudgetsTab";
import { telemetrySnippet } from "../pages/TelemetryCard";
import type { Budget, BudgetAlert } from "../api";
...
const forecast: UsageForecast = { monthToDateCost: 21, daysElapsedInMonth: 6, daysInMonth: 30, projectedMonthCost: 105, dailyAverage7: 3, dailyAverage30: 0.7, projectedNext30Cost: 90, trendPercent: 328.57, monthToDateTokens: 3_500_000, unpricedTokensMonthToDate: 200, projectedMonthSeasonal: 99, projectedMonthLow: 60, projectedMonthHigh: 140, projectedMonthCalibrated: 118.8, calibrationRatio: 1.2, cumulativeByDay: [3, 7, 10, 14, 18, 21] };
...
describe("ForecastTiles and LiveView", () => {
  it("shows month-to-date, projected month, next-30 and trend with their formulas", () => {
    ...
    expect(screen.getAllByText("$21.00").length).toBeGreaterThan(0);
    expect(screen.getAllByText("$99.00").length).toBeGreaterThan(0); // seasonal projection wins when present
    expect(screen.getAllByText("$118.80").length).toBeGreaterThan(0); // calibrated
    expect(screen.getAllByText(/likely range \$60\.00 – \$140\.00/).length).toBeGreaterThan(0);
...
describe("BudgetsView, AlertsList and telemetry snippets", () => {
  const budgets: Budget[] = [
    { id: "b1", scope: "tenant", scopeKey: null, monthlyAmount: 1000, name: "All AI", label: "All AI", enabled: true, updatedBy: "ana", updatedAtUtc: "2026-09-07T00:00:00Z", spentMonthToDate: 420, percent: 42, projectedMonth: 1800, projectedPercent: 180, state: "warn", daysElapsed: 7, daysInMonth: 30 },
    { id: "b2", scope: "team", scopeKey: "Payments", monthlyAmount: 200, name: null, label: "team: Payments", enabled: true, updatedBy: "ana", updatedAtUtc: "2026-09-07T00:00:00Z", spentMonthToDate: 260, percent: 130, projectedMonth: 1114, projectedPercent: 557, state: "over", daysElapsed: 7, daysInMonth: 30 },
  ];

  it("renders each budget as a bar with state, spent and projection", () => {
    const edited: string[] = [];
    const { container } = wrap(<BudgetsView budgets={budgets} onEdit={(b) => edited.push(b.id)} onRemove={() => {}} />);
    const view = within(container);
    expect(view.getByText("All AI")).toBeInTheDocument();
    expect(view.getByText("at risk")).toBeInTheDocument();
    expect(view.getByText("exceeded")).toBeInTheDocument();
    expect(view.getByText("Payments")).toBeInTheDocument();
    expect(view.getAllByRole("progressbar")).toHaveLength(2);
    expect(view.getByText(/projected \$1,800\.00 \(180%\)/)).toBeInTheDocument();
    fireEvent.click(view.getAllByText("Edit")[0]);
    expect(edited).toEqual(["b1"]);
  });

  it("lists alerts with kind and delivery state", () => {
    const alerts: BudgetAlert[] = [
      { id: "a1", budgetId: "b2", kind: "threshold", message: "Budget exceeded — team: Payments: 260.00 USD of 200.00 (130%)", amount: 260, percent: 130, createdAtUtc: "2026-09-07T10:00:00Z", delivered: false, deliveryError: "no notification channel configured" },
      { id: "a2", budgetId: null, kind: "anomaly", message: "Unusual AI spend today", amount: 20, percent: 1000, createdAtUtc: "2026-09-07T11:00:00Z", delivered: true, deliveryError: null },
    ];
    wrap(<AlertsList alerts={alerts} />);
    expect(screen.getByText("anomaly")).toBeInTheDocument();
    expect(screen.getByText(/not delivered: no notification channel configured/)).toBeInTheDocument();
    expect(screen.getByText(/delivered$/)).toBeInTheDocument();
  });

  it("builds sender snippets that point at the receiver with http/json and a bearer token", () => {
    const claude = telemetrySnippet("claude-code", "https://atlas.example.com");
    expect(claude).toContain("CLAUDE_CODE_ENABLE_TELEMETRY=1");
    expect(claude).toContain("OTEL_EXPORTER_OTLP_PROTOCOL=http/json");
    expect(claude).toContain("OTEL_EXPORTER_OTLP_ENDPOINT=https://atlas.example.com/api/ai-estate/otlp");
    expect(claude).toContain("Authorization=Bearer <ATLAS_TOKEN>");
    expect(telemetrySnippet("collector", "https://a")).toContain("encoding: json");
    expect(telemetrySnippet("litellm", "https://a")).toContain("/api/ai-estate/otlp/v1/traces");
    expect(telemetrySnippet("app", "https://a")).toContain("gen_ai.usage.input_tokens");
  });
});

describe("BurnUp and ReconciliationView", () => {
  it("draws the cumulative line with budget and projection guides", () => {
    const { container } = wrap(<BurnUp forecast={forecast} budget={120} lang="en" />);
    const view = within(container);
    expect(container.querySelector("polyline")).not.toBeNull();
    expect(container.querySelectorAll("line")).toHaveLength(2); // budget + projection guide
    expect(view.getByText(/projection \$99/)).toBeInTheDocument();
    expect(view.getByText(/budget \$120/)).toBeInTheDocument();
  });

  it("shows the ratio per provider and whether forecasts are calibrated", () => {
    const data: Reconciliation = {
      days: 30, from: "2026-08-08", to: "2026-09-07", blendedRatio: 1.2, method: "m",
      providers: [
        { provider: "anthropic", estimated: 110, reported: 150, daysWithBoth: 10, daysEstimatedOnly: 1, daysReportedOnly: 1, ratio: 1.2, ratioUsable: true, estimatedOnOverlap: 100, reportedOnOverlap: 120, note: "Estimate and billing agree within the expected range." },
        { provider: "openai", estimated: 15, reported: 12, daysWithBoth: 3, daysEstimatedOnly: 0, daysReportedOnly: 0, ratio: 0.8, ratioUsable: false, estimatedOnOverlap: 15, reportedOnOverlap: 12, note: "Only 3 day(s) overlap; 7 needed before the ratio is applied." },
      ],
    };
    const { container } = wrap(<ReconciliationView data={data} />);
    const view = within(container);
    expect(view.getByText(/Forecasts are calibrated ×1\.2/)).toBeInTheDocument();
    expect(view.getByText("×1.2")).toBeInTheDocument();
    expect(view.getByText("×0.8")).toBeInTheDocument();
    expect(view.getByText(/Only 3 day\(s\) overlap/)).toBeInTheDocument();
  });
});
