...
export interface CostProfile {
  ...
  updatedBy: string | null;
  updatedAtUtc: string | null;
  windowsHostingPerLegacyAppYear: number | null;
  extendedSupportPerLegacyAppYear: number | null;
  sqlServerSavingsPerYear: number | null;
}
...
export interface Portfolio {
  ...
  aiEstate?: PortfolioAiEstate | null;
}

// ---- AI Estate (governance module) ----

export interface AiEstateProvider {
  id: string;
  name: string;
  kind: string;
  packages: string[];
  endpointFiles: number;
  envVars: string[];
  models: string[];
  codeFiles: number;
...
export interface AiEstateMcpServer {
  config: string;
  name: string;
  transport: string;
  capability: string;
...
export interface AiEstateRecord {
  catalogVersion: string;
  catalogHash: string;
  providers: AiEstateProvider[];
  frameworks: { name: string; packages: string[] }[];
  mcp: AiEstateMcpServer[];
  activeModels: string[];
  retiredModels: { model: string; retiredOn: string | null; replacement: string | null; files: number }[];
  vectorStores: string[];
...
export interface AssessmentAiEstate {
  scanned: boolean;
  record: AiEstateRecord | null;
  openPii: number;
  openSecrets: number;
  aiRules: { ruleId: string; title: string; maxSeverity: string; count: number }[];
}
...
export interface AiCostProvider {
  ...
  currency: string;
  total: number;
  topDimensions: { key: string; amount: number }[];
...
export interface AiCostSummary {
  ...
  seats: { provider: string; total: number; active: number; idle: number; pendingCancellation: number; plan: string | null }[];
  activity: { provider: string; key: string; averagePerDay: number; unit: string }[];
  sourcesConfigured: number;
}

export interface PortfolioAiRow {
  assessmentId: string;
  name: string;
  providers: string[];
  frameworks: string[];
  ...
  secretsInMcp: number;
  piiCoLocated: boolean;
  secretsCoLocated: boolean;
  tags: string[] | null;
}

export interface PortfolioAiEstate {
  assessmentsWithAi: number;
  assessmentsScanned: number;
  providers: { id: string; name: string; kind: string; assessments: number; approved: boolean | null }[];
  frameworks: { name: string; count: number }[];
  mcpServers: number;
  remoteMcpServers: number;
  secretsInMcp: number;
  retiredModelReferences: number;
  allowlistConfigured: boolean;
  unapproved: { name: string; count: number }[];
  aiWithPii: number;
  aiWithSecrets: number;
  rows: PortfolioAiRow[];
  catalogVersion: string | null;
  costs: AiCostSummary | null;
}
...
export interface MigratableWaiver {
  findingId: string;
  ruleId: string;
  title: string;
  severity: string;
  ...
  waiverReason: string;
  waiverAuthor: string;
  waiverCreatedAtUtc: string;
  waiverExpiresAtUtc: string | null;
}

export type UsageForecast = { monthToDateCost: number; daysElapsedInMonth: number; daysInMonth: number; projectedMonthCost: number; dailyAverage7: number; dailyAverage30: number; projectedNext30Cost: number; trendPercent: number | null; monthToDateTokens: number; unpricedTokensMonthToDate: number; projectedMonthSeasonal: number; projectedMonthLow: number; projectedMonthHigh: number; projectedMonthCalibrated: number | null; calibrationRatio: number | null; cumulativeByDay: number[] };
export type ProviderReconciliation = { provider: string; estimated: number; reported: number; daysWithBoth: number; daysEstimatedOnly: number; daysReportedOnly: number; ratio: number | null; ratioUsable: boolean; estimatedOnOverlap: number; reportedOnOverlap: number; note: string };
export type Reconciliation = { days: number; from: string; to: string; providers: ProviderReconciliation[]; blendedRatio: number | null; method: string };
export type UsageActor = { actor: string; tools: string[]; sessions: number; requests: number; tokens: number; estimatedCost: number; unpricedTokens: number; lastReportUtc: string; lastPeriod: string; forecast: UsageForecast };
export type LiveActor = { actor: string; tools: string[]; lastReportUtc: string; activeNow: boolean; tokensToday: number; estimatedCostToday: number; requestsToday: number; tokensLastHour: number };
export type LiveUsage = { period: string; asOfUtc: string; activeMinutes: number; activeActors: number; reportingActorsToday: number; tokensToday: number; estimatedCostToday: number; tokensLastHour: number; actors: LiveActor[]; curve: { atUtc: string; tokens: number; estimatedCost: number }[] };
export type UsageModel = { model: string; tokens: number; estimatedCost: number | null; actors: number };
export type UsageDay = { period: string; tokens: number; estimatedCost: number; actors: number };
export type AiUsageSummary = {
  days: number; from: string; to: string; priceCatalogVersion: string; currency: string;
  reportingActors: number; totalTokens: number; estimatedCost: number; unpricedTokens: number;
  actors: UsageActor[]; byModel: UsageModel[]; byDay: UsageDay[]; forecast: UsageForecast;
  byProvider: { provider: string; tokens: number; estimatedCost: number; unpricedTokens: number; actors: number; models: number }[];
  byTool: { tool: string; tokens: number; estimatedCost: number; actors: number; source: string }[];
  byTeam: { team: string; members: number; activeMembers: number; tokens: number; estimatedCost: number; monthToDateCost: number; projectedMonthCost: number }[];
};
export type Budget = { id: string; scope: string; scopeKey: string | null; monthlyAmount: number; name: string | null; label: string; enabled: boolean; updatedBy: string; updatedAtUtc: string; spentMonthToDate: number; percent: number; projectedMonth: number; projectedPercent: number; state: "ok" | "warn" | "over"; daysElapsed: number; daysInMonth: number };
export type BudgetAlert = { id: string; budgetId: string | null; kind: "threshold" | "anomaly"; message: string; amount: number; percent: number | null; createdAtUtc: string; delivered: boolean; deliveryError: string | null };
export type Team = { id: string; name: string; members: string[]; updatedBy: string; updatedAtUtc: string; webhookUrl: string | null; slackWebhookUrl: string | null; teamsWebhookUrl: string | null };
export type CostSyncSchedule = { enabled: boolean; hourUtc: number; days: number; nextRunUtc: string | null; lastRunUtc: string | null; lastTenants: number; lastSources: number; lastFailures: number };
export type KnownActor = { actor: string; team: string | null; lastSeen: string };
export type ModelPrice = { id: string | null; pattern: string; input: number; output: number; cacheRead: number | null; cacheWrite: number | null; source: "tenant" | "builtin" | "config"; note: string | null; updatedBy: string | null; updatedAtUtc: string | null };
export type UnpricedModel = { model: string; tokens: number; actors: number; lastSeen: string };
export type PriceCatalog = { version: string; baseVersion: string; currency: string; note: string | null; prices: ModelPrice[]; unpriced: UnpricedModel[] };
export interface AiAllowlist {
  approvedProviders: string[];
  source: "tenant" | "config" | "none";
  updatedBy: string | null;
  updatedAtUtc: string | null;
}
...
export const api = {
  ...
  portfolioReportUrl: (lang: string, tag?: string) => withToken(`/api/portfolio/report?lang=${encodeURIComponent(lang)}${tag ? `&tag=${encodeURIComponent(tag)}` : ""}`),
  portfolioReportPdfUrl: (lang: string, tag?: string) => withToken(`/api/portfolio/report.pdf?lang=${encodeURIComponent(lang)}${tag ? `&tag=${encodeURIComponent(tag)}` : ""}`),
  ...
  createPolicy: (id: string, body: { rulePattern: string; pathGlob: string | null; reason: string; author: string; expiresAtUtc?: string | null }) =>
  ...
  getPortfolioTrend: (weeks = 26, tag?: string) =>
    http<PortfolioTrendPoint[]>(`/api/portfolio/trend?weeks=${weeks}${tag ? `&tag=${encodeURIComponent(tag)}` : ""}`),
  ...
  createTenant: (body: { name: string; externalKey: string | null }) =>
    http<Tenant>(`/api/tenants/`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
  getNotificationSettings: () => http<NotificationSettings>(`/api/settings/notifications`),
  setNotificationSettings: (body: NotificationSettings) =>
    http<NotificationSettings>(`/api/settings/notifications`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    }),
  jobsEventsUrl: () => withToken(`/api/events/jobs`),
  removeDemo: () => http<{ removed: number }>(`/api/demo`, { method: "DELETE" }),
  getCostProfile: () => http<CostProfile>(`/api/settings/cost`),
  setCostProfile: (body: { currency: string; hourlyRate: number; teamSize: number | null; author: string | null; windowsHostingPerLegacyAppYear?: number | null; extendedSupportPerLegacyAppYear?: number | null; sqlServerSavingsPerYear?: number | null }) =>
  ...
  // AI Estate (governance module)
  getAiEstate: async (lang: string, tag?: string): Promise<PortfolioAiEstate | null> =>
    (await http<PortfolioAiEstate | undefined>(`/api/ai-estate?lang=${encodeURIComponent(lang)}${tag ? `&tag=${encodeURIComponent(tag)}` : ""}`)) ?? null,
  getAssessmentAiEstate: (id: string, lang: string) => http<AssessmentAiEstate>(`/api/assessments/${id}/ai-estate?lang=${encodeURIComponent(lang)}`),
  listCostSources: () => http<CostSource[]>("/api/ai-estate/cost/sources"),
  costProviders: () => http<string[]>("/api/ai-estate/cost/providers"),
  upsertCostSource: (provider: string, body: { credentialName: string; scope: string | null; enabled: boolean }) =>
    http<CostSource>(`/api/ai-estate/cost/sources/${encodeURIComponent(provider)}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
  deleteCostSource: (provider: string) => http<void>(`/api/ai-estate/cost/sources/${encodeURIComponent(provider)}`, { method: "DELETE" }),
  syncCosts: (days: number) => http<CostSyncResult[]>("/api/ai-estate/cost/sync", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ days }) }),
  getCostSummary: async (days: number): Promise<AiCostSummary | null> => (await http<AiCostSummary | undefined>(`/api/ai-estate/cost?days=${days}`)) ?? null,
  getAllowlist: () => http<AiAllowlist>("/api/ai-estate/allowlist"),
  saveAllowlist: (approvedProviders: string[], author?: string) =>
    http<AiAllowlist>("/api/ai-estate/allowlist", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ approvedProviders, author: author ?? null }) }),
  clearAllowlist: () => http<void>("/api/ai-estate/allowlist", { method: "DELETE" }),
  catalogProviders: () => http<CatalogProvider[]>("/api/ai-estate/catalog/providers"),
  // Developer usage (opt-in CLI reports)
  getUsageSummary: async (days: number): Promise<AiUsageSummary | null> => (await http<AiUsageSummary | undefined>(`/api/ai-estate/usage?days=${days}`)) ?? null,
  getUsageLive: async (activeMinutes = 15): Promise<LiveUsage | null> => (await http<LiveUsage | undefined>(`/api/ai-estate/usage/live?activeMinutes=${activeMinutes}`)) ?? null,
  getUsagePrices: () => http<PriceCatalog>("/api/ai-estate/usage/prices"),
  getReconciliation: (days = 30) => http<Reconciliation>(`/api/ai-estate/usage/reconciliation?days=${days}`),
  // Budgets, alerts, teams
  listBudgets: () => http<Budget[]>("/api/ai-estate/budgets"),
  upsertBudget: (body: { id: string | null; scope: string; scopeKey: string | null; monthlyAmount: number; name: string | null; enabled: boolean; author?: string | null }) =>
    http<Budget>("/api/ai-estate/budgets", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
  deleteBudget: (id: string) => http<void>(`/api/ai-estate/budgets/${encodeURIComponent(id)}`, { method: "DELETE" }),
  budgetAlerts: (days = 30) => http<BudgetAlert[]>(`/api/ai-estate/budgets/alerts?days=${days}`),
  listTeams: () => http<Team[]>("/api/ai-estate/teams"),
  upsertTeam: (body: { id: string | null; name: string; members: string[]; author?: string | null; webhookUrl?: string | null; slackWebhookUrl?: string | null; teamsWebhookUrl?: string | null }) =>
    http<Team>("/api/ai-estate/teams", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
  deleteTeam: (id: string) => http<void>(`/api/ai-estate/teams/${encodeURIComponent(id)}`, { method: "DELETE" }),
  knownActors: () => http<KnownActor[]>("/api/ai-estate/teams/actors"),
  usageExportUrl: (days: number) => withToken(`/api/ai-estate/usage/export.csv?days=${days}`),
  costSchedule: () => http<CostSyncSchedule>("/api/ai-estate/cost/schedule"),
  upsertModelPrice: (body: { pattern: string; input: number; output: number; cacheRead: number | null; cacheWrite: number | null; note: string | null; author?: string | null }) =>
    http<ModelPrice>("/api/ai-estate/usage/prices", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
  deleteModelPrice: (id: string) => http<void>(`/api/ai-estate/usage/prices/${encodeURIComponent(id)}`, { method: "DELETE" }),
  // Waivers
  listSuppressions: (id: string) => http<Suppression[]>(`/api/assessments/${id}/suppressions`),
  listMigratableWaivers: (id: string) => http<MigratableWaiver[]>(`/api/assessments/${id}/suppressions/migratable`),
  migrateWaivers: (id: string, findingIds: string[] | null, author: string) =>
    http<{ migrated: number; skipped: number; migratedFindingIds: string[] }>(`/api/assessments/${id}/suppressions/migrate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ findingIds, author }),
    }),
