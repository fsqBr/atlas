...
import { AiEstatePanel } from "./AiEstatePanel";
...
export function AssessmentDetailPage() {
  ...
  const TABS: Tab[] = ["overview", "findings", "ai", "waivers", "runs", "modernization", "rules", "report", "settings"];
  ...
  const [runsForBaseline, setRunsForBaseline] = useState<{ id: string; number: number; finishedAtUtc: string | null }[]>([]);

  useEffect(() => {
    if (tab !== "report") return;
    api.listRuns(id).then((list) => setRunsForBaseline(list.filter((r) => r.status !== "Running" && r.finishedAtUtc).map((r) => ({ id: r.id, number: r.number, finishedAtUtc: r.finishedAtUtc }))))
      .catch(() => setRunsForBaseline([]));
  }, [tab, id, reportNonce]);
  ...
  return (
    <>
      ...
      {tab === "waivers" && <WaiversPanel assessmentId={id} onChanged={() => api.getHealth(id).then(setHealth)} />}
      {tab === "ai" && <AiEstatePanel assessmentId={id} refreshKey={`${assessment.completedAtUtc ?? ""}:${active}`} onGoTo={setTab} />}
      ...
      {tab === "report" && (
        <section className="card report">
          <div className="actions right">
            {runsForBaseline.length > 2 && (
              <label className="small muted" style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem" }}>
                {t("report.baseline")}
                <select value={baseline} onChange={(e) => setBaseline(e.target.value)} title={t("report.baselineHint")}>
                  <option value="">{t("report.baselinePrevious")}</option>
                  {runsForBaseline.slice(1).map((r) => (
                    <option key={r.id} value={r.finishedAtUtc!}>
                      #{r.number} · {formatDate(r.finishedAtUtc)}
                    </option>
                  ))}
                </select>
              </label>
            ...
            <a className="button" href={api.complianceUrl(id, lang)} title={t("report.complianceHint")}>
              {t("report.compliance")}
            </a>
            <a className="button" href={api.reportUrl(id, lang, baseline || undefined)} target="_blank" rel="noreferrer">
              {t("detail.openReport")} ↗
            </a>
            <a className="button primary" href={api.reportPdfUrl(id, lang, baseline || undefined)}>
          ...
          <iframe key={`${reportNonce}:${baseline}`} title="report" src={api.reportUrl(id, lang, baseline || undefined)} className="report-frame" />
        </section>
      )}
    </>
  );
