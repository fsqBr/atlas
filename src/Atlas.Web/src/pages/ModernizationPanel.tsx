    ...
    <>
      <section className="card">
        ...
        <div className="mod-hero">
          ...
          {plan.savings && plan.savings.annualTotal > 0 && (
            <div className="tile">
              <span className="tile-v">{money(plan.savings.annualTotal, plan.savings.currency, lang)}</span>
              <span className="tile-l">{t("mod.savingsYear")}</span>
            </div>
          )}
          {rec.paybackMonths != null && (
            <div className="tile">
              <span className="tile-v">{rec.paybackMonths} {t("mod.months")}</span>
              <span className="tile-l">{t("mod.payback")}</span>
            </div>
          )}
        </div>
      </section>
      ...
      {plan.savings && plan.savings.annualTotal > 0 && (
        <section className="card">
          <h2>{t("mod.savings")}</h2>
          <p className="muted small">{t("mod.savingsHint")}</p>
          <table>
            <tbody>
              {plan.savings.items.map((item) => (
                <tr key={item.key}>
                  <td>{item.label}</td>
                  <td className="num">{money(item.annualAmount, plan.savings!.currency, lang)}</td>
                </tr>
              ))}
              <tr>
                <td><strong>Σ {t("mod.savingsYear")}</strong></td>
                <td className="num"><strong>{money(plan.savings.annualTotal, plan.savings.currency, lang)}</strong></td>
              </tr>
            </tbody>
          </table>
          <p className="muted small">
            {plan.savings.assumptions.map((a) => `${a.label}: ${a.value}`).join(" · ")}
          </p>
        </section>
      )}
        ...
        <table className="mod-strategies">
          <thead>
            <tr>
              ...
              <th className="num">{t("mod.payback")}</th>
              <th />
            </tr>
          </thead>
          <tbody>
            {plan.strategies.map((s: StrategyInfo) => (
              <Fragment key={s.strategy}>
                <tr className={s.recommended ? "recommended" : ""}>
                  <td>
                  ...
                  <td className="num">{s.paybackMonths != null ? `${s.paybackMonths} ${t("mod.months")}` : "—"}</td>
                  <td>
