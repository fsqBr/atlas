...
import { api, type Calibration, type Portfolio, type PortfolioTrendPoint } from "../api";
import { ErrorBox, RiskBadge, ScoreBadge, SeverityChip, StatusChip } from "../components";
import { Donut, HBars, Legend, RangeBars, severityColor, TrendLine, useTokens } from "../components/charts";
...
export function PortfolioPage() {
  ...
  const [trend, setTrend] = useState<PortfolioTrendPoint[] | null>(null);
  const [trendMetric, setTrendMetric] = useState("Overall");
  ...
  useEffect(() => {
    ...
    api.getPortfolioTrend(26, tagFilter || undefined).then(setTrend).catch(() => setTrend([]));
  }, [lang, t, tagFilter]);
  ...
  return (
    <>
      ...
      <div className="actions" style={{ marginBottom: "0.8rem" }}>
        <a className="button" href={api.portfolioReportUrl(lang, tagFilter || undefined)} target="_blank" rel="noreferrer">
          {t("portfolio.report")}
        </a>
        <a className="button" href={api.portfolioReportPdfUrl(lang, tagFilter || undefined)}>{t("portfolio.reportPdf")}</a>
      </div>
        ...
        <div className="grid-2">
          <section className="card">
            <div className="actions" style={{ justifyContent: "space-between", alignItems: "baseline" }}>
              <h2>{t("portfolio.trendScore")}</h2>
              <select value={trendMetric} onChange={(e) => setTrendMetric(e.target.value)} title={t("portfolio.trendMetric")}>
                <option value="Overall">{t("portfolio.trendOverall")}</option>
                {(["Security", "Modernization", "Dependencies", "Architecture", "Quality"] as const).map((d) => (
                  <option key={d} value={d}>{t(`dim.${d}`)}</option>
                ))}
              </select>
            </div>
            <p className="muted small">{t("portfolio.trendHint")}</p>
            <TrendLine
              height="h-sm"
              points={trend.map((p) => ({
                x: p.date.slice(5),
                value: trendMetric === "Overall" ? p.averageScore : p.dimensions?.[trendMetric] ?? null,
                hint: `${formatDate(p.date)} · ${p.assessed} ${t("portfolio.assessments").toLowerCase()}`,
              }))}
            />
          </section>
          <section className="card">
            <h2>{t("portfolio.trendOpen")}</h2>
            <p className="muted small">{t("portfolio.trendHint")}</p>
            <TrendLine height="h-sm" domain={[0, Math.max(...trend.map((p) => p.openFindings)) * 1.1]} color="var-high" points={trend.map((p) => ({ x: p.date.slice(5), value: p.openFindings, hint: formatDate(p.date) }))} />
          </section>
        </div>
      )}
      ...
      {(() => {
        ...
        const groups = allTags.map((tag) => {
          const rows = data.rows.filter((r) => (r.tags ?? []).includes(tag));
          const scored = rows.filter((r) => r.score != null);
          return {
            tag,
            count: rows.length,
            avg: scored.length === 0 ? null : Math.round(scored.reduce((sum, r) => sum + (r.score ?? 0), 0) / scored.length),
            open: rows.reduce((sum, r) => sum + (r.openFindings ?? 0), 0),
          };
        });
        return (
          <section className="card">
              ...
              <table>
                <thead>
                  <tr>
                    <th>{t("tags.tag")}</th>
                    <th className="num">{t("portfolio.assessments")}</th>
                    <th className="num">{t("portfolio.avgScore")}</th>
                    <th className="num">{t("portfolio.openFindings")}</th>
                  </tr>
                </thead>
                <tbody>
                  {groups.map((g) => (
                    <tr key={g.tag}>
                      <td>
                        <button className="link" onClick={() => setTagFilter(tagFilter === g.tag ? "" : g.tag)}>
                          {tagFilter === g.tag ? "◉ " : ""}{g.tag}
                        </button>
                      </td>
                      <td className="num">{g.count}</td>
                      <td className="num">{g.avg ?? "—"}</td>
                      <td className="num">{formatNumber(g.open)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <section className="card">
        <h2>
          {t("portfolio.assessmentsTitle")}
          {tagFilter && (
            <button className="link small" style={{ marginLeft: "0.6rem" }} onClick={() => setTagFilter("")}>
              {t("tags.filtering", { tag: tagFilter })} ✕
            </button>
          )}
        </h2>
        <table>
          ...
          <tbody>
            {data.rows.filter((r) => !tagFilter || (r.tags ?? []).includes(tagFilter)).map((r) => (
              <tr key={r.id}>
                <td>
                  ...
                  <div className="muted small">
                    {r.sourceKind}
                    {(r.tags ?? []).map((tag) => (
                      <span key={tag} className="chip" style={{ marginLeft: "0.35rem" }}>{tag}</span>
                    ))}
                  </div>
                </td>
                <td>
