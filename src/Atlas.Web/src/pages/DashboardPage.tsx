...
import { api, type AssessmentSummary, type Portfolio, type AiUsageSummary, type Budget } from "../api";
...
export function DashboardPage() {
  ...
  const [aiUsage, setAiUsage] = useState<AiUsageSummary | null | undefined>(undefined);
  ...
  useEffect(() => {
    api.getUsageSummary(30).then(setAiUsage).catch(() => setAiUsage(null));
    api.listBudgets().then(setBudgets).catch(() => setBudgets(null));
  }, []);
  ...
  return (
    <>
      ...
      <Card title={t("dash.aiSpend")} subtitle={t("dash.aiSpendHint")} actions={<Link to="/ai-estate?tab=budgets" className="button small">{t("dash.aiOpen")} →</Link>}>
        {aiUsage === undefined ? (
          <Skeleton kind="tile" count={4} />
        ) : aiUsage === null ? (
          <p className="muted">{t("dash.aiSpendNone")} <Link to="/ai-estate?tab=usage">{t("dash.aiSpendSetup")} →</Link></p>
        ) : (
          <div className="kpis" style={{ margin: 0 }}>
            <Tile value={`$${aiUsage.forecast.monthToDateCost.toLocaleString(lang, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} label={t("usage.fc.mtd")} tone="neutral" hint={t("usage.fc.mtdHint", { d: aiUsage.forecast.daysElapsedInMonth, n: aiUsage.forecast.daysInMonth })} onClick={() => navigate("/ai-estate?tab=usage")} />
            <Tile value={`$${(aiUsage.forecast.projectedMonthSeasonal || aiUsage.forecast.projectedMonthCost).toLocaleString(lang, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} label={t("usage.fc.month")} tone="medium" hint={t("dash.aiProjectedHint")} />
            <Tile value={aiUsage.reportingActors} label={t("usage.devs")} tone="accent" hint={t("usage.devsHint", { days: aiUsage.days })} />
            <Tile value={budgets ? budgets.filter((b) => b.state !== "ok").length : "—"} label={t("dash.aiBudgetsAtRisk")} tone={budgets && budgets.some((b) => b.state === "over") ? "critical" : budgets && budgets.some((b) => b.state === "warn") ? "high" : "ok"} hint={budgets ? t("dash.aiBudgetsHint", { n: budgets.length }) : ""} onClick={() => navigate("/ai-estate?tab=budgets")} />
          </div>
        )}
      </Card>

      <Card title={t("dash.aiEstate")} subtitle={t("dash.aiEstateHint")} actions={<Link to="/ai-estate" className="button small">{t("dash.aiOpen")} →</Link>}>
        {!p ? (
          <Skeleton kind="tile" count={4} />
        ) : p.aiEstate && p.aiEstate.assessmentsWithAi > 0 ? (
          <div className="kpis" style={{ margin: 0 }}>
            <Tile value={p.aiEstate.assessmentsWithAi} label={t("aiestate.reposUsingAi")} tone="accent" hint={t("aiestate.scanned", { n: p.aiEstate.assessmentsScanned })} onClick={() => navigate("/ai-estate")} />
            <Tile value={p.aiEstate.providers.length} label={t("aiestate.providers")} tone="neutral" hint={p.aiEstate.providers.slice(0, 3).map((x) => x.name).join(", ")} />
            {p.aiEstate.allowlistConfigured && <Tile value={p.aiEstate.unapproved.reduce((s, u) => s + u.count, 0)} label={t("aiestate.unapproved")} tone={p.aiEstate.unapproved.length > 0 ? "critical" : "ok"} />}
            <Tile value={p.aiEstate.aiWithPii} label={t("aiestate.aiPii")} tone={p.aiEstate.aiWithPii > 0 ? "high" : "ok"} />
            <Tile value={p.aiEstate.aiWithSecrets} label={t("aiestate.aiSecrets")} tone={p.aiEstate.aiWithSecrets > 0 ? "critical" : "ok"} />
            <Tile value={p.aiEstate.mcpServers} label={t("aiestate.mcpServers")} tone={p.aiEstate.remoteMcpServers > 0 ? "medium" : "neutral"} />
          </div>
        ) : (
          <p className="muted">{t("dash.aiNone")}</p>
        )}
      </Card>
