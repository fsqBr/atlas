...
import { api, type AiCostSummary, type CostSource, type CostSyncResult, type CostSyncSchedule, type Credential, type PortfolioAiEstate } from "../api";
...
export function AiEstatePage() {
  ...
  const [schedule, setSchedule] = useState<CostSyncSchedule | null>(null);
  ...
  useEffect(() => {
    ...
    api.costSchedule().then(setSchedule).catch(() => setSchedule(null));
  }, [load]);
  ...
  return (
    <>
      ...
      >
        {schedule && sources && sources.some((s) => s.enabled) && (
          <p className="small muted">
            {schedule.enabled
              ? t("aiestate.schedule", { hour: String(schedule.hourUtc).padStart(2, "0"), days: schedule.days })
              : t("aiestate.scheduleOff")}
            {schedule.lastRunUtc && ` · ${t("aiestate.scheduleLast", { when: formatDate(schedule.lastRunUtc), sources: schedule.lastSources, failures: schedule.lastFailures })}`}
          </p>
        )}
        {syncResults && (
