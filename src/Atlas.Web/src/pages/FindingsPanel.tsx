...
export function FindingsPanel({ assessmentId, onTriaged }: { assessmentId: string; onTriaged?: () => void }) {
  ...
  const [issuesMessage, setIssuesMessage] = useState<string | null>(null);

  async function exportIssues() {
    const raw = window.prompt(t("issues.prompt"), "10");
    if (!raw) return;
    setIssuesBusy(true);
    setIssuesMessage(null);
    try {
      const result = await api.exportIssues(assessmentId, Math.max(1, Math.min(50, Number(raw) || 10)), lang);
      setIssuesMessage(t("issues.done", { created: result.created }) + (result.errors.length > 0 ? ` · ${result.errors[0]}` : ""));
    } catch (err) {
      setIssuesMessage(err instanceof Error ? err.message : String(err));
    } finally {
      setIssuesBusy(false);
    }
  }
      ...
      <div className="filters">
        ...
        <span className="export-row">
          ...
          <button
            ...
            disabled={issuesBusy}
            title={t("issues.hint")}
            onClick={() => void exportIssues()}
          >
            {issuesBusy ? t("issues.busy") : t("issues.button")}
          </button>
          {issuesMessage && <span className="muted small">{issuesMessage}</span>}
        </span>
      </div>

      <WaiversCard assessmentId={assessmentId} onMigrated={() => { setReload((r) => r + 1); onTriaged?.(); }} />
...
function TriageBar({
  ...
  async function submit(action: "Suppress" | "FalsePositive" | "Reopen") {
    ...
    try {
      ...
      const expires = action === "Suppress" && waiverDays && Number(waiverDays) > 0
        ? new Date(Date.now() + Number(waiverDays) * 86400000).toISOString()
        : null;
      const updated = await api.triage(assessmentId, finding.id, { action, reason: action === "Reopen" ? null : reason, author, expiresAtUtc: expires }, lang);
      onDone(updated);
      setMode(null);
      setReason("");
      setWaiverDays("");
        ...
        >
          ...
          {mode === "Suppress" && (
            <input
              ...
              placeholder={t("triage.expiresDays")}
              title={t("triage.expiresHint")}
