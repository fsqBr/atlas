...
export function TeamsCard({ teams, onChanged }: { teams: Team[]; onChanged: () => void }) {
  ...
  async function save(e: FormEvent) {
    ...
    try {
      await api.upsertTeam({ id: editing, name: name.trim(), members: all });
      setEditing(null);
      setName("");
      setMembers([]);
      setExtra("");
    ...
    <Card title={t("teams.title")} subtitle={t("teams.hint")}>
          ...
          <tbody>
            {teams.map((tm) => (
              <tr key={tm.id}>
                ...
                <td>{tm.members.map((m) => <span key={m} className="kind">{m}</span>)}</td>
                <td>
                  <div className="actions">
      ...
      <form className="form price-form" onSubmit={save}>
        ...
        <div className="actions">
          ...
          {editing && <button type="button" className="button" onClick={() => { setEditing(null); setName(""); setMembers([]); setExtra(""); }}>{t("prices.clear")}</button>}
        </div>
      </form>
    </Card>
  );
...
export function BreakdownCard({ usage }: { usage: AiUsageSummary }) {
  ...
  return (
    <Card title={t("breakdown.title")} subtitle={t("breakdown.hint", { days: usage.days })}>
      <div className="tiles">
