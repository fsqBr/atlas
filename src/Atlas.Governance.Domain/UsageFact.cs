...
public sealed class UsageFact
{
    ...
    /// <summary>cli | otel | import.</summary>
    public string Source { get; private set; } = null!;

    /// <summary>Provider id (anthropic, openai, google…) from telemetry or guessed from the model; null when unknown.</summary>
    public string? Provider { get; private set; }

    /// <summary>What the sender itself reported as cost (Claude Code's cost metric, gen_ai.usage.cost) — informative, kept apart from the estimate.</summary>
    public decimal? ReportedCost { get; private set; }

    public void SetProvider(string? provider) => Provider = string.IsNullOrWhiteSpace(provider) ? null : provider.Length <= 40 ? provider : provider[..40];

    /// <summary>Adds an increment (telemetry). Sessions are a lower bound: distinct ids seen in the batch.</summary>
    public void Add(long inputTokens, long outputTokens, long cacheReadTokens, long cacheWriteTokens, int requests, int sessions, decimal? reportedCost)
    {
        ...
        Sessions = Math.Max(Sessions, sessions);
        if (reportedCost is not null)
        {
            ReportedCost = (ReportedCost ?? 0m) + reportedCost.Value;
        }
