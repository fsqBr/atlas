...
public sealed partial class QualityScanner : IScanner
{
    public static class RuleIds
    {
        ...
        public const string DeadCode = "quality.deadcode.type";
        public const string DeadMember = "quality.deadcode.member";
    }

    private const int MaxDuplicationFindings = 200;
    private const int MaxApiFindings = 500;
    private const int MaxDeadCodeFindings = 200;
    ...
    public ScannerDescriptor Descriptor { get; } = new(
        ...
        Capabilities: ["test-discovery", "coverage-ingestion", "complexity", "file-size", "duplication", "legacy-apis", "dead-code"]);
    ...
    [
        ...
        new(RuleIds.DeadCode, RulesVersion, FindingCategory.Quality, Severity.Informational,
            "Dead code candidate (unreferenced internal type)", "An internal type has no references anywhere in the analyzed source. Reflection, DI-by-convention and serialization are invisible to static analysis, so this is a candidate, not a verdict.",
            "Confirm with IDE find-usages plus a search for reflection/DI/serialization use; if truly unused, delete it — dead code still costs reading, builds and upgrades.",
            Loc(new("Candidato a código morto (tipo internal sem referências)", "Um tipo internal não tem referências em nenhum lugar do código analisado. Reflexão, DI por convenção e serialização são invisíveis à análise estática, então isto é um candidato, não um veredito.",
                "Confirme com find-usages da IDE e busca por uso via reflexão/DI/serialização; se realmente não usado, exclua — código morto ainda custa leitura, build e upgrades.",
                "Candidato a código morto: {typeName} em {fileName}", "{detail}. Uso via reflexão, DI por convenção ou serialização não pode ser descartado — verifique antes de excluir."))),
        new(RuleIds.DeadMember, RulesVersion, FindingCategory.Quality, Severity.Informational,
            "Dead code candidate (unreferenced private member)", "A private method, field, property or event has no references anywhere in the analyzed source — not even in its own type. Reflection, serialization and data binding are invisible to static analysis, so this is a candidate, not a verdict.",
            "Confirm with IDE find-usages plus a search for reflection/serialization/binding use; if truly unused, delete it.",
            Loc(new("Candidato a código morto (membro privado sem referências)", "Um método, campo, propriedade ou evento privado não tem referências em nenhum lugar do código analisado — nem no próprio tipo. Reflexão, serialização e data binding são invisíveis à análise estática, então isto é um candidato, não um veredito.",
                "Confirme com find-usages da IDE e busca por uso via reflexão/serialização/binding; se realmente não usado, exclua.",
                "Candidato a código morto: {typeName} em {fileName}", "{detail}. Uso via reflexão, serialização ou binding não pode ser descartado — verifique antes de excluir."))),
    ];
    ...
    public async Task<ScanResult> ExecuteAsync(ScanContext context, CancellationToken cancellationToken)
    {
        ...
        EmitDeadCode(context, languages.SelectMany(l => l.Patterns));
    ...
    /// <summary>
    /// Dead-code candidates found by the language adapters (top-level internal types with no
    /// source references). Informational and Medium-confidence on purpose: reflection,
    /// DI-by-convention and serialization cannot be ruled out statically.
    /// </summary>
    private static void EmitDeadCode(ScanContext context, IEnumerable<PatternFact> patterns)
    {
        var candidates = patterns
            .Where(p => p.PatternId is QualityPatternIds.DeadType or QualityPatternIds.DeadMember)
            .GroupBy(p => (p.PatternId, p.FilePath, p.Symbol))
            .Select(g => g.OrderBy(p => p.Line).First())
            .OrderBy(p => p.FilePath, StringComparer.Ordinal).ThenBy(p => p.Symbol, StringComparer.Ordinal)
            .GroupBy(p => p.PatternId)
            .SelectMany(g => g.Take(MaxDeadCodeFindings));

        foreach (var fact in candidates)
        {
            var fileName = Path.GetFileName(fact.FilePath);
            context.Findings.Emit(new FindingCandidate(
                fact.PatternId == QualityPatternIds.DeadType ? RuleIds.DeadCode : RuleIds.DeadMember,
                Severity.Informational, ConfidenceLevel.Medium,
                Title: $"Dead code candidate: {fact.Symbol} in {fileName}",
                Message: $"{fact.Detail}. Reflection, DI-by-convention or serialization use cannot be ruled out statically — verify before deleting.",
                Evidence: new EvidenceCandidate(FilePath: fact.FilePath, LineStart: fact.Line, Symbol: $"{fileName}:{fact.Symbol}"),
                Remediation: "Confirm with IDE find-usages plus a search for reflection/DI/serialization use; if truly unused, delete it.",
                Data: new Dictionary<string, string> { ["typeName"] = fact.Symbol, ["fileName"] = fileName, ["detail"] = fact.Detail }));
        }
    }
    ...
    private static void EmitTestFacts(ScanContext context, IReadOnlyList<ProjectFact> projects, IReadOnlyList<FileFact> files)
    {
        ...
        // A test that references the composition root exercises the layers beneath it:
        // coverage-by-reference walks the reference closure, not just the direct edge.
        var pending = new Queue<string>(covered);
        while (pending.TryDequeue(out var coveredPath))
        {
            if (!byPath.TryGetValue(Normalize(coveredPath), out var current))
            {
                continue;
            }

            var currentDir = Path.GetDirectoryName(current.RelativePath) ?? string.Empty;
            foreach (var reference in current.ProjectReferences)
            {
                var resolved = Normalize(Path.GetRelativePath(".", Path.Combine(currentDir, reference.Replace('\\', Path.DirectorySeparatorChar))));
                if (byPath.TryGetValue(resolved, out var target) && covered.Add(target.RelativePath))
                {
                    pending.Enqueue(target.RelativePath);
                }
            }
        }
    ...
    {
        var allReports = await CoberturaParser.FindAndParseAsync(context.Workspace, cancellationToken);
        // coverlet configured with "cobertura,opencover" writes the same run twice, and stale copies
        // under TestResults/ repeat it again: identical content must produce one summary, not N.
        var reports = allReports
            .DistinctBy(r => (Math.Round(r.LineRate, 4), r.Packages.Count,
                string.Join("|", r.Packages.OrderBy(p => p.Name, StringComparer.Ordinal).Select(p => FormattableString.Invariant($"{p.Name}:{p.LineRate:F4}")))))
            .ToList();
        if (reports.Count == 0)
        {
            context.Findings.Emit(new FindingCandidate(
    ...
    /// <summary>Packages that carry a test-framework name but are referenced by production code
    /// (extensibility contracts, embedded runners): never proof of a test project.</summary>
    private static readonly string[] NotTestPackages = ["xunit.abstractions", "xunit.extensibility", "nunit.engine"];

    public static bool IsTestProject(ProjectFact project) =>
        TestProjectName().IsMatch(project.Name)
        || project.PackageReferences.Any(p =>
            !NotTestPackages.Any(x => p.Id.StartsWith(x, StringComparison.OrdinalIgnoreCase))
            && TestPackageMarkers.Any(m => p.Id.Equals(m, StringComparison.OrdinalIgnoreCase) || p.Id.StartsWith(m + ".", StringComparison.OrdinalIgnoreCase)))
