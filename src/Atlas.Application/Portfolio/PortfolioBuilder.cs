...
public sealed record PortfolioSummary(
    ...
    PortfolioAiEstate? AiEstate = null);
...
public sealed class PortfolioBuilder(
    ...
    IRuleCatalog rules,
    IAiCostSummarySource? costs = null)
{
    private const int CostWindowDays = 30;
    private const int MaxAssessments = 500;
    private const int TopRules = 15;

    public async Task<PortfolioSummary> BuildAsync(string? lang, CancellationToken cancellationToken, string? tag = null)
    {
        var list = await assessments.ListRecentAsync(MaxAssessments, cancellationToken);
        if (!string.IsNullOrWhiteSpace(tag))
        {
            var wanted = tag.Trim();
            list = list.Where(a => a.Tags?.Any(t => string.Equals(t, wanted, StringComparison.OrdinalIgnoreCase)) == true).ToList();
        }
        ...
        var aiInventories = ids.Count == 0 ? new Dictionary<Guid, string?>() : await findings.GetLatestOccurrenceDataByRuleAsync(ids, AiEstateRuleIds.Inventory, cancellationToken);
        var aiCosts = costs is null ? null : await costs.GetAsync(CostWindowDays, cancellationToken);
        if (aiCosts is { IsEmpty: true })
        {
            aiCosts = null;
        }
        ...
        return new PortfolioSummary(
            ...
            Benchmark: benchmark,
            Targets: targets,
            AiEstate: aiInventories.Count == 0 && aiCosts is null ? null : PortfolioAiEstate.Build(list, aiInventories, open) with { Costs = aiCosts });
    }
}
