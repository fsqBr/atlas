...
public static class PortfolioTrend
{
    ...
    public static IReadOnlyList<PortfolioTrendPoint> Compute(IReadOnlyList<CompletedRunPoint> runs, DateOnly today, int weeks = DefaultWeeks)
    {
        ...
        List<DateOnly> dates;
        if (today.DayNumber - first.DayNumber < 14)
        {
            // A young estate has no weeks yet: sample daily so the chart lives from day one.
            dates = Enumerable.Range(0, today.DayNumber - first.DayNumber + 1).Select(first.AddDays).ToList();
        }
        else
        {
            dates = Enumerable.Range(0, weeks)
