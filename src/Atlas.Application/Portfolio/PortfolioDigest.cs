...
public static class PortfolioDigestBuilder
{
        ...
        DateTimeOffset now)
    {
        ...
        // Week-over-week deltas compare only assessments present at BOTH endpoints — an assessment
        // onboarded this week is not a "regression" of the estate, and must not swing the deltas.
        var both = current.Keys.Where(weekAgo.ContainsKey).ToList();
        ...
        var pairedNow = both.Where(k => current[k].HealthScore is not null).Select(k => (double)current[k].HealthScore!).ToList();
        var pairedThen = both.Where(k => weekAgo[k].HealthScore is not null).Select(k => (double)weekAgo[k].HealthScore!).ToList();
        var averageDelta = pairedNow.Count == 0 || pairedThen.Count == 0
            ? (int?)null
            : (int)Math.Round(pairedNow.Average() - pairedThen.Average());

        var open = current.Values.Sum(p => p.OpenFindings ?? 0);
        var openDelta = both.Count == 0
            ? (int?)null
            : both.Sum(k => current[k].OpenFindings ?? 0) - both.Sum(k => weekAgo[k].OpenFindings ?? 0);
