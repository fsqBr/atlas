    ...
    ITenantContext tenant)
{
    ...
    public async Task<(SuppressionPolicy Policy, int Applied)> CreateAsync(
        Guid? assessmentId, string rulePattern, string? pathGlob, string reason, string author, CancellationToken cancellationToken)
    {
        var policy = new SuppressionPolicy(Guid.NewGuid(), tenant.Require(), assessmentId, rulePattern, pathGlob, reason, author);
        policies.Add(policy);
        ...
        if (assessmentId is { } recompute && applied > 0)
        {
        ...
        IReadOnlyList<Atlas.Scanner.Abstractions.FindingCandidate> candidates, IReadOnlyList<SuppressionPolicy> active, out int dropped)
    {
        ...
        var kept = candidates.Where(c => !active.Any(p => p.Matches(c.RuleId, c.Evidence.FilePath))).ToList();
    ...
    private async Task<int> ApplyToExistingAsync(Guid assessmentId, SuppressionPolicy policy, CancellationToken cancellationToken)
    {
        ...
        foreach (var item in page.Items.Concat(regressed.Items))
        {
            ...
            suppressions.Add(new FindingSuppression(
                Guid.NewGuid(), item.Finding, SuppressionKind.Suppressed, $"Policy {policy.Describe()}: {policy.Reason}", policy.Author));
            applied++;
        }
