...
public static class FindingReconciler
{
        ...
        bool scanSucceeded)
    {
        ...
        foreach (var candidate in candidates)
        {
            ...
            {
                // Rule major bump: the same location fingerprinted under the previous major
                // identifies the finding this one supersedes. The old finding resolves via the normal
                // sweep; the new one starts Open (a rule that changed meaning demands re-triage), and
                // the link lets triage offer migrating the predecessor's waiver instead of losing it.
                string? predecessor = null;
                if (rule.MajorVersion > 1)
                {
                    var previous = FindingFingerprint.Compute(
                        candidate.RuleId,
                        rule.MajorVersion - 1,
                        repositoryKey,
                        candidate.Evidence.FilePath,
                        candidate.Evidence.Symbol ?? candidate.Evidence.SnippetHash);
                    if (byFingerprint.ContainsKey(previous))
                    {
                ...
                finding = Finding.Create(
                    ...
                    FindingOrigin.Deterministic, scanId, predecessor);
