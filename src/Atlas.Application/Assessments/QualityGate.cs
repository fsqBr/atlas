...
public static class QualityGate
{
        ...
        IReadOnlyDictionary<Severity, int>? newBySeverity = null)
    {
        ...
        if (newThreshold is { } nt && newBySeverity is not null)
        {
            ...
            {
                violations.Add($"{offendingNew.Sum(kv => kv.Value)} new finding(s) introduced by the latest run at severity {nt} or above: " + string.Join(", ", offendingNew.Select(kv => $"{kv.Key} {kv.Value}")) + ".");
            }
        }
