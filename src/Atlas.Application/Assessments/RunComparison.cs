...
public sealed record RunComparison(
    ...
    IReadOnlyDictionary<Severity, int>? RegressedBySeverity = null);
...
public static class RunDiff
{
        ...
        string? lang = null)
    {
        ...
        return new RunComparison(
            ...
            inventory,
            created.GroupBy(f => f.Finding.Severity).ToDictionary(g => g.Key, g => g.Count()),
            regressed.GroupBy(f => f.Finding.Severity).ToDictionary(g => g.Key, g => g.Count()));
    }
    ...
    IRuleCatalog rules)
{
    ...
    public async Task<RunComparison?> BuildAsync(Guid assessmentId, Guid runId, Guid? withRunId, string? lang, CancellationToken cancellationToken)
    {
        ...
        var previous = withRunId is { } w
            ...
            : all.Where(r => r.Number < current.Number && r.Status is AssessmentRunStatus.Completed or AssessmentRunStatus.CompletedWithWarnings)
