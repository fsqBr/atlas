...
public interface IAssessmentRunRepository
{
    ...
    /// <summary>Every health snapshot in the tenant (runs and triage recomputes), reduced to the trend's
    /// needs; an optional tag narrows it to the assessments carrying that label (product groups).</summary>
    Task<IReadOnlyList<Atlas.Application.Portfolio.CompletedRunPoint>> ListCompletedPointsAsync(string? tag, CancellationToken cancellationToken);
...
public interface IFindingRepository
{
    ...
    /// <summary>Suppressed findings whose newest active suppression carries an expiry in the past (waiver ended).</summary>
    Task<IReadOnlyList<Finding>> ListExpiredSuppressedAsync(DateTimeOffset now, CancellationToken cancellationToken);

    /// <summary>Untriaged findings created by a rule major bump: they carry the superseded fingerprint.</summary>
    Task<IReadOnlyList<Finding>> ListWithPredecessorAsync(Guid assessmentId, CancellationToken cancellationToken);

    /// <summary>Latest occurrence data (JSON) of the open/regressed finding of one rule per assessment — how the portfolio reads per-repository inventories (e.g. ai.inventory) without loading findings.</summary>
    Task<IReadOnlyDictionary<Guid, string?>> GetLatestOccurrenceDataByRuleAsync(IReadOnlyCollection<Guid> assessmentIds, string ruleId, CancellationToken cancellationToken);
...
public interface IRuleCatalog
{
    ...
    /// <summary>Removes a scanner's rules from the catalog. Demo cleanup only: real scanners keep
    /// retired rules on purpose.</summary>
    Task DeleteScannerRulesAsync(string scannerId, CancellationToken cancellationToken);
}

/// <summary>Per-tenant severity tuning of catalog rules (applied to candidates at scan time).</summary>
public interface IRuleOverrideRepository
{
    Task<IReadOnlyList<Atlas.Domain.Rules.RuleSeverityOverride>> ListAsync(CancellationToken cancellationToken);

    /// <summary>ruleId → severity for one tenant, bypassing the ambient tenant filter (the worker has none).</summary>
    Task<IReadOnlyDictionary<string, Severity>> MapForTenantAsync(Guid tenantId, CancellationToken cancellationToken);

    Task<Atlas.Domain.Rules.RuleSeverityOverride?> GetAsync(string ruleId, CancellationToken cancellationToken);

    void Add(Atlas.Domain.Rules.RuleSeverityOverride @override);

    void Remove(Atlas.Domain.Rules.RuleSeverityOverride @override);
}

/// <summary>The tenant's market parameters for cost.v1 (currency, hourly rate, team size).</summary>
public interface ITenantCostProfileRepository
{
    /// <summary>One tenant's profile, always by explicit tenant id (the worker has no ambient tenant,
    /// and an ambient-filtered SingleOrDefault would misbehave under the system scope).</summary>
    Task<Atlas.Domain.Modernization.TenantCostProfile?> GetForTenantAsync(Guid tenantId, CancellationToken cancellationToken);

    void Add(Atlas.Domain.Modernization.TenantCostProfile profile);

    void Remove(Atlas.Domain.Modernization.TenantCostProfile profile);
}

/// <summary>Durable key/value for cross-replica coordination (digest dedupe and friends).</summary>
public interface ISystemMarkerRepository
{
    Task<Atlas.Domain.Operations.SystemMarker?> GetAsync(string key, CancellationToken cancellationToken);

    Task SetAsync(string key, string value, CancellationToken cancellationToken);

    /// <summary>
    /// Atomically claims a marker: sets it to <paramref name="value"/> and returns true only if it
    /// was absent or last written before <paramref name="staleBefore"/>. Two worker replicas racing
    /// on the same key — the weekly digest — get exactly one winner, so the estate is not notified
    /// twice. Commits on its own (raw upsert), independent of the ambient unit of work.
    /// </summary>
    Task<bool> TryClaimAsync(string key, string value, DateTimeOffset staleBefore, CancellationToken cancellationToken);
}

/// <summary>Per-tenant notification channels (webhook, Slack/Teams, weekly digest).</summary>
public interface ITenantNotificationSettingsRepository
{
    Task<Atlas.Domain.Tenants.TenantNotificationSettings?> GetForTenantAsync(Guid tenantId, CancellationToken cancellationToken);

    Task<IReadOnlyList<Atlas.Domain.Tenants.TenantNotificationSettings>> ListAllAsync(CancellationToken cancellationToken);

    void Add(Atlas.Domain.Tenants.TenantNotificationSettings settings);

    void Remove(Atlas.Domain.Tenants.TenantNotificationSettings settings);
}
...
public interface IScanJobQueue
{
    ...
    /// <summary>
    /// Renews the lease of a job this worker still owns (Leased/Running). Returns false when the
    /// row was already reclaimed by another replica — the caller should abandon the work to avoid
    /// a duplicate run. Atomic, on its own connection: safe to call from a background heartbeat.
    /// </summary>
    Task<bool> HeartbeatAsync(Guid jobId, string workerId, TimeSpan leaseDuration, CancellationToken cancellationToken);
