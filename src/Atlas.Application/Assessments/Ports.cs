...
{
    /// <summary>The current tenant's profile (ambient tenant filter applies).</summary>
    Task<Atlas.Domain.Modernization.TenantCostProfile?> GetAsync(CancellationToken cancellationToken);

    /// <summary>One tenant's profile, bypassing the ambient filter (the worker computes plans without one).</summary>
