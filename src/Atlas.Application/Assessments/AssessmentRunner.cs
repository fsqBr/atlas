...
public sealed class AssessmentRunner(
    ...
    Atlas.Application.AiEstate.ITenantAiEstateSettingsRepository? aiEstateSettings = null)
{
    public async Task RunAsync(Guid assessmentId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Tenant settings scanners need travel with the request: the scan host has no database.
            var settings = new Dictionary<string, string>(StringComparer.Ordinal);
            var aiAllowlist = aiEstateSettings is null ? null : await aiEstateSettings.GetForTenantAsync(assessment.TenantId, cancellationToken);
            if (aiAllowlist is { HasAllowlist: true })
            {
                settings[ScanSettingKeys.AiApprovedProviders] = aiAllowlist.ToSetting();
            }
            ...
            if (projectCount == 0)
            {
                // Non-.NET estates (Java/Python) declare no projects, so approximate estate size from
                // source files — otherwise a large Java/Python monorepo is scored as a ≤10-project
                // estate (no size normalization) and unfairly penalized. ~20 files ≈ one project.
                var fileCount = outcome.Languages.Values.Sum(l => l.Totals.FileCount);
                projectCount = fileCount > 0 ? Math.Max(1, (int)Math.Round(fileCount / 20.0)) : 0;
            }
