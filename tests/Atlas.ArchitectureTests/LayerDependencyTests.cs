...
public class LayerDependencyTests
{
    ...
    [Fact]
    public void Core_never_depends_on_the_governance_module()
    {
        foreach (var assembly in new[] { Domain, Application, Contracts, typeof(Atlas.Reporting.ExecutiveReportBuilder).Assembly, typeof(Atlas.Infrastructure.Persistence.AtlasDbContext).Assembly })
        {
            var result = Types.InAssembly(assembly).ShouldNot().HaveDependencyOn("Atlas.Governance").GetResult();
            Assert.True(result.IsSuccessful, $"{assembly.GetName().Name}: {FailureList(result)}");
        }
    }

    [Fact]
    public void Governance_layers_respect_the_same_rules_as_the_core()
    {
        var domain = Types.InAssembly(typeof(Atlas.Governance.Domain.CostSource).Assembly)
            .ShouldNot().HaveDependencyOnAny("Atlas.Application", "Atlas.Governance.Application", "Atlas.Governance.Infrastructure", "Atlas.Infrastructure", "Atlas.Api", "Microsoft.EntityFrameworkCore", "Npgsql")
            .GetResult();
        Assert.True(domain.IsSuccessful, "Governance.Domain: " + FailureList(domain));

        var application = Types.InAssembly(typeof(Atlas.Governance.Application.CostSyncService).Assembly)
            .ShouldNot().HaveDependencyOnAny("Atlas.Governance.Infrastructure", "Atlas.Infrastructure", "Atlas.Api", "Atlas.Worker", "Microsoft.EntityFrameworkCore", "Microsoft.AspNetCore", "Npgsql")
            .GetResult();
        Assert.True(application.IsSuccessful, "Governance.Application: " + FailureList(application));

        var contracts = Types.InAssembly(typeof(Atlas.Governance.Contracts.CostSourceResponse).Assembly)
            .ShouldNot().HaveDependencyOnAny("Atlas.Domain", "Atlas.Application", "Atlas.Governance.Domain", "Atlas.Governance.Application", "Atlas.Infrastructure", "Atlas.Api")
            .GetResult();
        Assert.True(contracts.IsSuccessful, "Governance.Contracts: " + FailureList(contracts));
    }
