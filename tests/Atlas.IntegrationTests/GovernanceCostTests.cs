...
public sealed class GovernanceCostTests(PostgresFixture fixture) : IClassFixture<PostgresFixture>
{
    ...
    public async Task Cost_source_sync_and_summary_flow_through_the_governance_schema()
    {
        ...
        usage = await client.GetFromJsonAsync<UsageSummaryRow>("/api/ai-estate/usage?days=30");
        var otelActor = Assert.Single(usage!.Actors, a => a.Actor.StartsWith("id-", StringComparison.Ordinal));
        ...
        // Team channels must be https; a valid Slack hook is stored and echoed back.
        var teamRow = (await team.Content.ReadFromJsonAsync<TeamRow>())!;
        Assert.Equal(HttpStatusCode.BadRequest, (await client.PutAsJsonAsync("/api/ai-estate/teams", new { id = teamRow.Id, name = "Platform", members = new[] { "dev-1" }, slackWebhookUrl = "http://insecure.example/x" })).StatusCode);
        var withChannel = await client.PutAsJsonAsync("/api/ai-estate/teams", new { id = teamRow.Id, name = "Platform", members = new[] { "dev-1" }, slackWebhookUrl = "https://hooks.slack.com/services/T/B/x" });
        Assert.Equal(HttpStatusCode.OK, withChannel.StatusCode);
        Assert.Contains("hooks.slack.com", await withChannel.Content.ReadAsStringAsync());

        // Finance export and the daily sync status.
        var csv = await client.GetAsync("/api/ai-estate/usage/export.csv?days=30");
        Assert.Equal(HttpStatusCode.OK, csv.StatusCode);
        Assert.Contains("text/csv", csv.Content.Headers.ContentType?.MediaType);
        var csvText = await csv.Content.ReadAsStringAsync();
        Assert.Contains("period,actor,team,tool,provider,model", csvText);
        Assert.Contains("dev-1,Platform,claude-code", csvText);
        Assert.Equal(HttpStatusCode.OK, (await client.GetAsync("/api/ai-estate/cost/schedule")).StatusCode);

        // Reconciliation: usage exists, no billing for these providers yet → rows with notes, no usable ratio.
        var recon = await client.GetFromJsonAsync<ReconRow>("/api/ai-estate/usage/reconciliation?days=30");
        Assert.Null(recon!.BlendedRatio);
        Assert.Contains(recon.Providers, p => p.Provider == "anthropic" && p.Ratio == null);
        // Cursor is a known cost source without a scope requirement.
        Assert.Equal(HttpStatusCode.OK, (await client.PutAsJsonAsync("/api/ai-estate/cost/sources/cursor", new { credentialName = "openai-admin" })).StatusCode);
        Assert.Contains("cursor", (await client.GetFromJsonAsync<List<string>>("/api/ai-estate/cost/providers"))!);
        Assert.Equal(HttpStatusCode.NoContent, (await client.DeleteAsync("/api/ai-estate/cost/sources/cursor")).StatusCode);
        Assert.Equal(HttpStatusCode.NoContent, (await client.DeleteAsync($"/api/ai-estate/budgets/{platform.Id}")).StatusCode);
        Assert.Equal(HttpStatusCode.NoContent, (await client.DeleteAsync($"/api/ai-estate/teams/{teamRow.Id}")).StatusCode);
    }

    private sealed record KnownActorRow(string Actor, string? Team);

    private sealed record ReconProviderRow(string Provider, decimal? Ratio);

    private sealed record ReconRow(decimal? BlendedRatio, List<ReconProviderRow> Providers);
