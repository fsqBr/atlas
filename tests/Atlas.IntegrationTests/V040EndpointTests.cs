...
public sealed class V040EndpointTests(PostgresFixture fixture) : IClassFixture<PostgresFixture>
{
    ...
    public async Task Cost_profile_round_trips_and_resets()
    {
        ...
        var put = await client.PutAsJsonAsync("/api/settings/cost", new { currency = "usd", hourlyRate = 120, teamSize = 6, author = "cfo", windowsHostingPerLegacyAppYear = 900, sqlServerSavingsPerYear = 4000 });
        ...
        // Per-currency savings rates round-trip so a foreign-currency tenant can get ROI in its currency.
        Assert.Equal(900, saved.GetProperty("windowsHostingPerLegacyAppYear").GetDecimal());
        Assert.Equal(4000, saved.GetProperty("sqlServerSavingsPerYear").GetDecimal());
        Assert.Equal(System.Text.Json.JsonValueKind.Null, saved.GetProperty("extendedSupportPerLegacyAppYear").ValueKind);

        Assert.Equal(HttpStatusCode.BadRequest, (await client.PutAsJsonAsync("/api/settings/cost", new { currency = "USD", hourlyRate = 120, windowsHostingPerLegacyAppYear = -5 })).StatusCode);
