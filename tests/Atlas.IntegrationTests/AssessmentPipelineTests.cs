...
public sealed class AssessmentPipelineTests(PostgresFixture fixture) : IClassFixture<PostgresFixture>, IDisposable
{
    ...
    [Fact]
    public async Task Heartbeat_renews_only_for_the_owning_worker_and_blocks_reclaim()
    {
        await using var provider = fixture.BuildServices();
        ...
        {
            var handler = scope.ServiceProvider.GetRequiredService<CreateAssessmentHandler>();
            await handler.HandleAsync("Heartbeat test", new SourceReference(SourceReference.Kinds.LocalFolder, _source), CancellationToken.None);
        }

        // Claim with a lease already in the past, then heartbeat it forward: a second claimer must
        // no longer be able to reclaim it.
        using var scopeA = provider.CreateScope();
        var queueA = scopeA.ServiceProvider.GetRequiredService<IScanJobQueue>();
        var job = await queueA.ClaimAsync("worker-a", TimeSpan.FromMilliseconds(-1), CancellationToken.None);
        Assert.NotNull(job);

        // A different worker cannot renew a lease it does not own.
        using (var scopeC = provider.CreateScope())
        {
            Assert.False(await scopeC.ServiceProvider.GetRequiredService<IScanJobQueue>()
                .HeartbeatAsync(job.Id, "worker-c", TimeSpan.FromMinutes(30), CancellationToken.None));
        }

        Assert.True(await queueA.HeartbeatAsync(job.Id, "worker-a", TimeSpan.FromMinutes(30), CancellationToken.None));

        using var scopeB = provider.CreateScope();
        var reclaimed = await scopeB.ServiceProvider.GetRequiredService<IScanJobQueue>()
            .ClaimAsync("worker-b", TimeSpan.FromMinutes(5), CancellationToken.None);
        Assert.Null(reclaimed); // the fresh heartbeat kept worker-a's claim
    }
