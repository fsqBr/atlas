...
public class AesGcmSecretCipherTests
{
    ...
    [Theory]
    [InlineData("Y2ktbWFzdGVyLWtleS0zMi1ieXRlcy1wbGFjZWhvbGRlciEh", "36")] // 36 bytes: the v0.58.0 CI placeholder
    [InlineData("c2hvcnQ=", "5")]
    public void A_key_that_is_not_32_bytes_is_rejected_with_the_actual_length(string key, string decodedLength)
    {
        var ex = Assert.Throws<InvalidOperationException>(() => new AesGcmSecretCipher(new SecretCipherOptions { MasterKeyBase64 = key }));
        Assert.Contains("32 bytes", ex.Message);
        Assert.Contains($"decodes to {decodedLength}", ex.Message);
        Assert.Contains("openssl rand -base64 32", ex.Message);
    }

    [Fact]
    public void A_key_that_is_not_base64_is_rejected_with_a_clear_message()
    {
        var ex = Assert.Throws<InvalidOperationException>(() => new AesGcmSecretCipher(new SecretCipherOptions { MasterKeyBase64 = "base64-32-bytes-change-me" }));
        Assert.Contains("not valid base64", ex.Message);
    }

    [Fact]
    public void A_blank_key_means_not_configured_rather_than_an_error()
    {
        Assert.False(new AesGcmSecretCipher(new SecretCipherOptions { MasterKeyBase64 = "  " }).IsConfigured);
    }
}
