...
public sealed class LocalSourcesAndUploadTests(PostgresFixture fixture) : IClassFixture<PostgresFixture>, IDisposable
{
    ...
    public async Task Upload_stores_archive_and_manifest_and_rejects_garbage()
    {
        ...
        {
            using var writer = new StreamWriter(zip.CreateEntry("Uploaded/Uploaded.csproj").Open());
            writer.Write("<Project />");
        }

        using var form = new MultipartFormDataContent
        {
            { new StringContent("Uploaded.App"), "name" },
            { new StringContent("1"), "files" },
            { new ByteArrayContent(zipStream.ToArray()), "archive", "Uploaded.App.zip" },
        };
        ...
        Assert.Equal("Uploaded.App", uploaded!.Name);
    ...
    public async Task Replacing_the_upload_swaps_the_archive_deletes_the_old_one_and_queues_a_run()
    {
        ...
        var created = await client.PostAsJsonAsync("/api/assessments", new { name = "Uploaded", sourceKind = "upload", sourceLocator = first.UploadId.ToString(), branch = (string?)null });
    ...
    private static async Task<Uploaded> UploadAsync(HttpClient client, string marker)
    {
        ...
        using var form = new MultipartFormDataContent
        {
            { new StringContent("Uploaded"), "name" },
            { new StringContent("1"), "files" },
            { new ByteArrayContent(zipStream.ToArray()), "archive", "Uploaded.zip" },
        };
        var response = await client.PostAsync("/api/uploads", form);
