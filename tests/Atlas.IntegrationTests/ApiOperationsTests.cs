...
public sealed class ApiOperationsTests(PostgresFixture fixture) : IClassFixture<PostgresFixture>
{
    ...
    public void Rbac_maps_paths_to_roles()
    {
        ...
        // Reads are viewer-open except the administrative inventories, which are admin-only even for GET.
        Assert.False(Atlas.Api.AuthSetup.RequiresRoleCheck("GET", "/api/assessments"));
        Assert.False(Atlas.Api.AuthSetup.RequiresRoleCheck("GET", "/api/ai-estate/usage"));
        Assert.True(Atlas.Api.AuthSetup.RequiresRoleCheck("POST", "/api/assessments"));
        Assert.True(Atlas.Api.AuthSetup.RequiresRoleCheck("GET", "/api/tokens"));
        Assert.True(Atlas.Api.AuthSetup.RequiresRoleCheck("GET", "/api/credentials"));
        Assert.True(Atlas.Api.AuthSetup.RequiresRoleCheck("GET", "/api/tenants"));
        Assert.Equal("atlas-admin", Atlas.Api.AuthSetup.RequiredRole(options, "GET", "/api/tokens"));
        Assert.Equal("atlas-admin", Atlas.Api.AuthSetup.RequiredRole(options, "GET", "/api/credentials"));
    ...
    [Theory]
    [InlineData("2026-09-07T05:59:00Z", 6, "2026-09-07T06:00:00Z")]
    [InlineData("2026-09-07T06:00:00Z", 6, "2026-09-08T06:00:00Z")]
    [InlineData("2026-09-07T23:30:00Z", 0, "2026-09-08T00:00:00Z")]
    public void Scheduler_picks_the_next_daily_slot(string now, int hour, string expected)
    {
        Assert.Equal(DateTimeOffset.Parse(expected), Atlas.Api.CostSyncSchedulerService.NextRun(DateTimeOffset.Parse(now), hour));
    }
}
