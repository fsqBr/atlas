...
public class DeadCodeEmitTests : IDisposable
{
    ...
    [Fact]
    public async Task Dead_member_patterns_map_to_the_member_rule()
    {
        var candidates = await ScanAsync(
            new PatternFact(QualityPatternIds.DeadMember, "src/App/Svc.cs", 8, "Svc.Orphan", "private method 'Svc.Orphan' has no source references in the analyzed code"));

        var dead = Assert.Single(candidates, c => c.RuleId == QualityScanner.RuleIds.DeadMember);
        Assert.Equal("Dead code candidate: Svc.Orphan in Svc.cs", dead.Title);
        Assert.Equal(Severity.Informational, dead.Severity);
