...
public class RuleAuditWalkerTests
{
    ...
    [Fact]
    public void Conditional_access_invocations_do_not_crash_the_walker_and_still_match_weak_hashes()
    {
        // "?.Create()" is a member binding whose text starts with '.'; before v0.50 the tail-segment search went
        // below index zero and the exception took the whole scan host down (every scanner failed for the run).
        var facts = Security("""
            using System.Security.Cryptography;
            public class Hashing
            {
                public byte[] Weak(byte[] data, HashAlgorithm? algo, object? factory)
                {
                    var h = MD5.Create();
                    var viaConditional = System.Security.Cryptography.MD5.Create()?.ComputeHash(data);
                    factory?.ToString();
