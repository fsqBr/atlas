...
public class FindingReconcilerTests
{
    ...
    private static IReadOnlyDictionary<string, RuleDefinition> BumpedRules() => new Dictionary<string, RuleDefinition>(Rules)
    {
        ["dep.eol"] = new("dep.eol", "dep", "2.0.0", FindingCategory.Modernization, Severity.High, "EOL (stricter)", "desc", null),
    };

    [Fact]
    public void Rule_major_bump_links_the_new_finding_to_the_one_it_supersedes()
    {
        var first = Run(Guid.NewGuid(), [Candidate("dep.eol", "a.csproj", "net45")], []);
        var superseded = first.Created[0];
        Assert.Null(superseded.PredecessorFingerprint);

        var second = FindingReconciler.Reconcile(
            Tenant, Assessment, Guid.NewGuid(), "dep", "0.1.0", Repo,
            [Candidate("dep.eol", "a.csproj", "net45")], BumpedRules(), first.Created, scanSucceeded: true);

        // New identity, Open (the rule changed meaning: re-triage), and a link back to what it replaces.
        var successor = Assert.Single(second.Created);
        Assert.NotEqual(superseded.Fingerprint, successor.Fingerprint);
        Assert.Equal(superseded.Fingerprint, successor.PredecessorFingerprint);
        Assert.Equal(FindingStatus.Open, successor.Status);

        // The superseded finding is absent under the new major: the normal sweep resolves it.
        Assert.Equal(1, second.Resolved);
        Assert.Equal(FindingStatus.Resolved, superseded.Status);
    }

    [Fact]
    public void Rule_major_bump_without_a_prior_match_has_no_predecessor()
    {
        var result = FindingReconciler.Reconcile(
            Tenant, Assessment, Guid.NewGuid(), "dep", "0.1.0", Repo,
            [Candidate("dep.eol", "a.csproj", "net45")], BumpedRules(), [], scanSucceeded: true);

        Assert.Null(Assert.Single(result.Created).PredecessorFingerprint);
    }

    [Fact]
    public void Rule_major_bump_does_not_link_across_a_different_location()
    {
        var first = Run(Guid.NewGuid(), [Candidate("dep.eol", "a.csproj", "net45")], []);

        var second = FindingReconciler.Reconcile(
            Tenant, Assessment, Guid.NewGuid(), "dep", "0.1.0", Repo,
            [Candidate("dep.eol", "b.csproj", "net45")], BumpedRules(), first.Created, scanSucceeded: true);

        Assert.Null(Assert.Single(second.Created).PredecessorFingerprint);
    }
}
