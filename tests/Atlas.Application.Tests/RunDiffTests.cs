...
public class RunDiffTests
{
    ...
    public void Classifies_new_resolved_and_regressed_by_scan_membership()
    {
        ...
        // The baseline gate must see reintroduced findings: RegressedBySeverity is counted per finding.
        Assert.NotNull(result.RegressedBySeverity);
        Assert.Equal(regressed.Count, result.RegressedBySeverity!.Values.Sum());
