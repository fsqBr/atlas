...
public class MigrationPlanTests
{
    ...
    [Fact]
    public void Payback_beyond_a_ten_year_horizon_is_hidden_rather_than_shown_as_centuries()
    {
        var basePlan = SamplePlan();
        var estimate = basePlan.Estimates.First(e => e.Strategy != ModernizationStrategy.KeepStabilize);

        // Tiny annual savings against a large modernization cost → hundreds of years of "payback".
        var tinySavings = new SavingsEstimate("savings.v1", [], AnnualTotal: 100m, "BRL", []);
        var plan = basePlan with { Savings = tinySavings };
        Assert.Null(plan.PaybackMonths(estimate)); // not "3789.5 Months"

        // A realistic ratio still yields a number.
        var realSavings = new SavingsEstimate("savings.v1", [], AnnualTotal: estimate.Cost.Likely / 2m, "BRL", []);
        Assert.NotNull((basePlan with { Savings = realSavings }).PaybackMonths(estimate));
    }

    [Fact]
    public void Partial_strategies_pay_back_slower_because_they_retire_less_of_the_legacy_estate()
    {
        var estimate = SamplePlan().Estimates.First(e => e.Strategy != ModernizationStrategy.KeepStabilize);
        // Annual savings == the cost so a full retirement pays back in ~12 months, well within horizon.
        var plan = SamplePlan() with { Savings = new SavingsEstimate("savings.v1", [], AnnualTotal: estimate.Cost.Likely, "BRL", []) };

        // Same cost estimate, only the strategy (and thus the captured savings share) differs.
        var full = plan.PaybackMonths(estimate with { Strategy = ModernizationStrategy.FullRewrite });
        var incremental = plan.PaybackMonths(estimate with { Strategy = ModernizationStrategy.Incremental });

        Assert.NotNull(full);
        Assert.NotNull(incremental);
        Assert.True(incremental!.Value > full!.Value, $"incremental {incremental} should pay back slower than full {full}");
    }
