...
`atlas-agent install --server … --token … --every 12` sends once, saves the settings under `~/.atlas-agent` and
registers a per-user schedule with the OS scheduler (Task Scheduler, launchd or cron); `atlas-agent status` and
`atlas-agent uninstall` show and remove it. There is still no service and no resident process: the scheduler starts the
agent, it reports, it exits, and the server cannot configure it.

The *Developer usage & prices* tab on the AI estate page shows who reported, tokens, estimated cost, a per-developer
average, a 30-day run rate, per-model and per-day breakdowns, and the step-by-step guide. Prices are USD per million
