...
- **Automation** — scheduled re-runs, signed webhooks, API tokens, a CI quality gate with ready-made GitHub Action and
...
docker compose up -d # first start builds the images: 5–10 minutes; later starts take seconds
```
...
| Portfolio | `GET /api/portfolio`, `GET /api/calibration` |
...
GitHub Action `.github/actions/atlas-scan` (example in `deploy/ci/examples/github-atlas.yml`) and the Azure DevOps
template `deploy/ci/azure-pipelines-atlas.yml`. Pipelines authenticate with an **API token** (`atlas_pat_…`, created
...
- Webhooks carry scores and counts, never finding contents, and are HMAC-SHA256 signed.
...
| `Atlas:Notifications` | `WebhookUrl`, `Secret`, `PublicBaseUrl` |
