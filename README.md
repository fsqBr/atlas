...
The rule catalog is in the open — every rule, its live counts in your estate, and per-tenant severity tuning:

![Rule catalog](docs/images/rules.png)

| Portfolio trend (overall and per dimension) | Tenant cost model — your market, your currency |
|---|---|
| ![Portfolio trend](docs/images/portfolio-trend.png) | ![Cost model](docs/images/cost.png) |

The business case in one screen — what modernizing costs, what staying legacy costs every year, and the payback:

![Modernization business case](docs/images/savings.png)

A Java estate gets the same treatment — JDK end-of-life, legacy frameworks, `javax.*` migration surface, metrics and security patterns:

![Java assessment](docs/images/java.png)

And so does Python — interpreter floors, end-of-life frameworks (Django 1.x, PyCrypto…), pinned-dependency CVEs, metrics and security patterns:

![Python assessment](docs/images/python.png)
...
Re-running is idempotent (a day's line is replaced, never counted twice). The *Developer usage* card on the AI estate
page shows who reported, tokens, estimated cost from a versioned list-price catalog (`ai-prices.json`; unknown models
are shown as *unpriced*, never as zero; `Atlas__AiEstate__PriceCatalogPath` replaces the catalog), a per-developer
average, a 30-day run rate, per-model and per-day breakdowns, and the step-by-step guide. The same page explains, per
provider, how to obtain the admin key a cost source needs. The CLI never installs itself and never runs unattended —
schedule it yourself if you want weekly reports.
