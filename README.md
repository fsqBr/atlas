...
10. [AI Estate (governance module)](#ai-estate-governance-module)
11. [Web UI](#web-ui)
...
The **AI estate** page shows what AI the portfolio uses (providers with approval status, agent frameworks, MCP servers,
per-repository flags for unapproved providers and AI next to personal data or secrets), the spend and seats pulled
from the connected cost sources, and the cost-source management itself (connect a provider through a stored
credential, sync, disconnect). Each assessment has an **AI estate** tab with the same facts for that repository, and
a **Waivers** tab gathers every human decision on its findings (active waivers with reopen, expired and reopened history,
standing policies with an inline form, waivers to migrate after a rule's major bump). The **approved AI providers** list
is edited on the AI estate page and applies to every assessment on its next run; the deployment configuration is only
the fallback.
...
Atlas is in active development and used in pilots; the current line is **v0.40**. What is deliberately *not* there
