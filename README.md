...
| Portfolio | `GET /api/portfolio`, `GET /api/calibration`, `GET /api/portfolio/trend?weeks=` (overall + per dimension) |
...
schedule, targets), compare, queue, the rule catalog with severity tuning, credentials, AI and API tokens. English and Portuguese, light/dark/system theme,
...
| `Atlas:Notifications` | `WebhookUrl`, `Secret`, `PublicBaseUrl`, `SlackWebhookUrl`, `TeamsWebhookUrl`, `DigestDayOfWeek`, `DigestHourUtc` |
