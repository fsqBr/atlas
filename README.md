...
| Quality | `quality.*` | Cyclomatic complexity, duplicated blocks, absent tests, coverage from Cobertura/OpenCover reports, APIs gone or discouraged on modern .NET, `[Obsolete]` usage, dead-code candidates (internal types nothing references) |
...
**Does it find dead code?** It flags *candidates*: top-level `internal` types that nothing in the analyzed
source references. Framework-instantiated shapes (controllers, handlers, migrations, workers…), attributed
types and entry points are skipped, and the findings stay Informational because reflection, DI-by-convention
and serialization can't be seen statically — confirm with find-usages before deleting.
